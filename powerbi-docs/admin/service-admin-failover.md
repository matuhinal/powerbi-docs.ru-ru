...
После запуска отработки отказа Power BI использует георепликацию хранилища Azure для выполнения отработки отказа. Такие репликации обычно имеют точку возврата, равную 15 минутам, но [служба хранилища Azure не гарантирует этот период](/azure/storage/common/storage-redundancy) в соглашении об уровне обслуживания, поэтому Power BI также не может гарантировать определенный период времени. 
...
В случае возникновения в основном регионе экстремальной аварии, которая не позволяет вернуть экземпляр в оперативный режим на продолжительное время, основной регион отработки отказа позволит выполнять операции чтения и записи, а клиенты могут повторно развернуть и настроить шлюзы в новом регионе.

Клиенты могут выбрать установку нового шлюза на другом компьютере или использовать существующий шлюз. Работа с существующим шлюзом не должна представлять трудностей, так как все источники данных, связанные со старым шлюзом, будут перенесены на новый.
