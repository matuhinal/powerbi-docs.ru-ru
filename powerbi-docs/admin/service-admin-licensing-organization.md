...
1. Выберите **Автоматически назначить всем пользователям без лицензий**, а затем оформите заказ.
  ...
  ![Подписки Power BI (бесплатная версия) с автоматическим назначением](media/service-admin-licensing-organization/m365-auto-assign.png) 
...
- [Find Power BI users that have signed in](service-admin-access-usage.md) (Поиск пользователей Power BI, которые выполнили вход в систему)
