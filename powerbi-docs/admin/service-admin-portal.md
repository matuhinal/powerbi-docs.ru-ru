...
- **Удалить все имеющиеся данные отдельных пользователей в текущем содержимом метрик использования** . Это действие применяется для блокирования доступа к данным метрик использования для всех корпоративных пользователей, которые уже могли работать с ними.
...
![Параметры клиента](media/service-admin-portal/powerbi-admin-tenant-settings-2.png)
...
* **Включено для части организации**. Эту функцию разрешено использовать определенным группам безопасности в вашей организации.
...
* **Обучение.** По умолчанию эта ссылка в меню "Справка" позволяет перейти к [списку всех схем и модулей обучения по Power BI](/learn/browse/?products=power-bi). Чтобы эта ссылка вела на внутренние ресурсы обучения, задайте пользовательский URL-адрес для параметра **Обучающая документация**.
...
Параметр **Allow users to try Power BI Pro** (Возможность пробного использования Power BI Pro) включен по умолчанию. Этот параметр обеспечивает больший контроль над тем, как пользователи получают лицензии на Power BI Pro. В сценариях с блокировкой самостоятельной покупки этот параметр позволяет начать пробное использование Power BI Pro. Возможности для пользователей зависят от сочетания параметров лицензий. В приведенной ниже таблице показано, как меняется возможность перехода с бесплатной версии Power BI на версию Power BI Pro в зависимости от разных сочетаний параметров:

| Параметр самостоятельной покупки | Параметр "Возможность пробного использования Power BI Pro" | Взаимодействие с конечным пользователем |
| ------ | ------ | ----- |
...
> [!NOTE]
> Для запросов на получение лицензии вы можете добавить внутренний URL-адрес, как описано в разделе [Параметры справки и поддержки](#help-and-support-settings). Указав такой URL-адрес, вы переопределите стандартный процесс самостоятельной покупки. Он не перенаправляет регистрацию для пробной версии лицензии Power BI Pro. Пользователи, которые могут приобрести лицензию в сценариях, описанных в таблице выше, будут перенаправляться на внутренний URL-адрес.
...
:::image type="content" source="media/service-admin-portal/power-bi-admin-datasets-workspaces.png" alt-text="Создание нового интерфейса рабочей области":::

Дополнительные сведения см. в статье [Общие сведения об использовании наборов данных в рабочих областях](../connect-data/service-datasets-across-workspaces.md).
...
![Приглашать внешних пользователей в организацию](media/service-admin-portal/powerbi-admin-allow-invite-aad-b2b-guests.png)
...
С помощью функции **Опубликовать в Интернете** администратор Power BI может предоставить пользователям возможность создавать коды внедрения для публикации отчетов в Интернете. Эта функция делает отчет и содержащиеся в нем данные доступными всем пользователям в Интернете. Дополнительные сведения о [публикации в Интернете](../collaborate-share/service-publish-to-web.md).
...
>[!IMPORTANT]
> До появления параметра "Экспорт в Excel" экспортом данных в файлы Excel управлял параметр "Экспорт данных". Поэтому в клиентах, существовавших до появления параметра "Экспорт в Excel", при первом обращении к нему администраторы Power BI будут видеть *несохраненные изменения*. Чтобы новый параметр вступил в силу, им необходимо применить эти изменения. В противном случае экспорт в файл Excel будет по прежнему контролироваться параметром "Экспорт данных".
...
Пользователи в организации могут использовать визуализацию ArcGIS Maps for Power BI от Esri. [Дополнительные сведения](../visuals/power-bi-visualizations-arcgis.md)
...
В разделе **Параметры клиента** параметр **Разрешить подключения к рекомендуемым таблицам** позволяет администраторам Power BI управлять тем, кто в организации может использовать рекомендуемые таблицы из коллекции типов данных Excel. 

:::image type="content" source="media/service-admin-portal/admin-allow-connections-featured-tables.png" alt-text="Создание нового интерфейса рабочей области":::

Подключения к рекомендуемым таблицам также отключаются, если для параметра **Экспорт данных** задано значение **Отключено**.
...
![Параметр публикации приложений-шаблонов включен для всей организации](media/service-admin-portal/power-bi-admin-portal-template-app-settings.png)
...
![Установка параметра приложений-шаблонов](media/service-admin-portal/power-bi-admin-portal-template-app-settings-installer-appsource.png)
...
![Установка параметра приложений-шаблонов, не перечисленных в AppSource](media/service-admin-portal/power-bi-admin-portal-template-app-settings-installer-nonappsource.png)
...
Администратор Power BI может управлять всеми отчетами, панелями мониторинга и приложениями, которые были добавлены в раздел "Рекомендуемое" на главной странице Power BI в организации.
