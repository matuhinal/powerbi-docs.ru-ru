...
### <a name="allow-giving-permissions-to-existing-azure-active-directory-guest-users"></a>Разрешить предоставлять разрешения существующим гостевым пользователям Azure Active Directory

Если этот параметр включен, пользователи в организации могут предоставлять разрешения отдельным гостевым пользователям в рамках процессов предоставления разрешений или общего доступа в Power BI. Если этот параметр отключен для пользователя, пользователь не может назначать разрешения гостевым пользователям или приглашать их в Power BI.

![Разрешить предоставлять разрешения существующим гостевым пользователям Azure Active Directory](media/service-admin-portal/powerbi-admin-allow-grant-access-to-aad-b2b-guests.png)


> [!IMPORTANT]
>  Этот параметр не запрещает назначать разрешения гостевым пользователям во всех случаях. Он ограничивает предоставление доступа лишь отдельным гостевым пользователям. Предоставлять доступ гостевым пользователям можно через группы пользователей, например группы безопасности Office 365, или списки рассылки. 

Если пользователь, которому запрещено предоставлять разрешения гостевым пользователям, попытается дать разрешение, он увидит в пользовательском интерфейсе сообщение об ошибке. Кроме того, при изменении разрешений на доступ к элементу пользователям, которым запрещено предоставлять разрешения гостям, должны удалить всех гостевых пользователей из списка доступа, прежде чем они смогут предоставить или изменить разрешения для этого элемента. 
...
Если параметр **Разрешить предоставлять разрешения существующим гостевым пользователям Azure Active Directory** отключен для пользователя, этот пользователь не сможет приглашать внешних пользователей в организацию с помощью Power BI.
