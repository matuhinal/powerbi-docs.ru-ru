...
*Рекомендуемые таблицы* позволяют связывать данные в Excel с данными в Power BI. Они упрощают добавление корпоративных данных на листы Excel. В Excel в коллекции типов данных находятся данные из рекомендуемых таблиц в наборах данных Power BI. В этой статье описано, как это сделать.

Создаете ли вы наборы данных в Power BI? Узнайте, как [настраивать рекомендуемые таблицы в Power BI Desktop](service-create-excel-featured-tables.md).

> [!NOTE]
> В Excel можно также анализировать данные из любого набора данных, доступного в Power BI. Дополнительные сведения см. в разделе [Другие способы доступа к наборам данных Power BI из Excel](service-analyze-in-excel.md#other-ways-to-access-power-bi-datasets-from-excel) в статье "Анализ в Excel данных Power BI".
> 
...
## <a name="search-for-power-bi-data-in-the-data-types-gallery"></a>Поиск данных Power BI с помощью коллекции "Типы данных"
...
:::image type="content" source="media/service-excel-featured-tables/excel-data-selector-store.png" alt-text="Снимок экрана: Организационные данные в Excel, таблица типов данных Поставщики.":::
 
Если в Excel найдена совпадающая с высокой достоверностью строка, ячейки связываются непосредственно с ней. Значок связанного элемента указывает на то, что ячейки связаны со строкой в Power BI.
...
Если ячейка содержит более одной потенциально совпадающей строки, в ячейке отображается значок вопросительного знака, и отображается область **Выбор данных**. В приведенном ниже примере мы выбрали диапазон "B3: B9", а затем выбрали рекомендованную таблицу Power BI **Магазин**. Все строки соответствуют, за исключением ячейки B9 "508 — Pasadena Lindseys". В области **Выбор данных** показаны два возможных совпадения: в двух разных таблицах значения совпадают.

:::image type="content" source="media/service-excel-featured-tables/excel-question-mark-featured-table.png" alt-text="Снимок экрана: панель выбора данных Excel.":::
...
Для каждого предложения выберите имя строки в области **Выбор данных**, чтобы просмотреть дополнительные сведения в строке, которые помогут выбрать нужную. Найдя строку, нажмите **Выбрать**, чтобы связать ее с ячейкой в Excel. 
...
## <a name="explore-related-data"></a>Изучение связанных данных

Теперь, когда вы создали связь между значениями на листе Excel и данными в рекомендованной таблице Power BI, можно изучить эти данные. Используйте данные для улучшения отчетов Excel.

### <a name="view-related-data-in-a-card"></a>Просмотр связанных данных на карточке 
 
Щелкните значок **Карточка** в ячейке, чтобы открыть карточку с данными из всевозможных полей и вычисляемых полей в рекомендуемой таблице. В заголовке карточки указывается значение поля метки строки в рекомендуемой таблице.
...
### <a name="insert-related-data-from-power-bi"></a>Вставка связанных данных из Power BI 
...
### <a name="use-cell-formulas"></a>Использование формул ячеек

В таблице Excel можно ссылаться на связанный столбец таблицы, а затем добавлять поля данных, используя ссылку `.` (точку).
...
Аналогично, в ячейке можно ссылаться на ячейку и использовать ссылку `.` (точку) для получения полей.
...
- **Тип данных** > **Преобразовать в текст**.
- **Тип данных** > **Изменить**.
- **Обновление**.
...
Вы будете видеть только рекомендуемые таблицы из наборов данных, для которых у вас есть разрешение в Power BI. При обновлении данных для получения строк необходимо иметь разрешение на доступ к набору данных в Power BI. В Power BI требуется [разрешение на сборку или запись в наборе данных](../connect-data/service-datasets-build-permissions.md).
 
Excel кэширует данные, возвращаемые для всей строки. Любой пользователь, которому вы предоставили общий доступ к файлу Excel, может видеть данные всех полей во всех связанных ячейках.
...
- При использовании для поиска кнопки **Организационные данные** Excel выполняет поиск только среди рекомендуемых таблиц в наборах данных Power BI.
