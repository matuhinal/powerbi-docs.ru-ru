...
ms.locfileid: "49641765"
---
# <a name="power-bi---basic-concepts-for-power-bi-service-consumers"></a>Power BI — основные понятия для ***потребителей*** службы Power BI
...
Потребителю Power BI доступно пять стандартных блоков: ***визуализации***, ***панели мониторинга***, ***отчеты***, ***приложения*** и  ***наборы данных***. Они иногда называются ***содержимым*** *Power BI*. А *содержимое* находится в ***рабочих областях***. Эти блоки применяются в стандартном рабочем процессе: *конструктор* Power BI (выделен желтым цветом на приведенной ниже схеме) собирает данные из *наборов данных*, вводит их в Power BI для анализа, создает  *отчеты* с *визуализациями*, которые представляют интересные факты и данные, закрепляет визуализации из отчетов на панель мониторинга и предоставляет общий доступ к отчетам и панелям *потребителям*, таким как вы (выделено черным цветом на схеме ниже), в виде *приложений* или других типов общего содержимого. 
...
Доступны также визуализации сообщества, которые называются *пользовательскими визуальными элементами*. Если вы получите отчет с незнакомым визуальным элементом, скорее всего, это будет пользовательский визуальный элемент. Если вам нужна помощь в интерпретации пользовательского визуального элемента, <!--[look up the name of the report or dashboard *designer*](end-user-owner.md)-->найдите имя *конструктора* отчета или панели мониторинга и свяжитесь с ним.
