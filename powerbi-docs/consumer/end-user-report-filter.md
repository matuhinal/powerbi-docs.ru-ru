...
description: Добавление фильтра в отчет в службе Power BI для бизнес-пользователей
author: mihart
ms.reviewer: mihart
ms.custom: ''
ms.service: powerbi
...
Существует много различных способов фильтрации данных в Power BI. В этой статье описывается использование панели **Фильтры**.  Вы также можете выполнять фильтрацию, выбирая точки данных в визуальном элементе отчета для фильтрации других визуальных элементов на странице. Этот процесс называется **перекрестной фильтрацией** и **перекрестным выделением**. Дополнительные сведения о перекрестной фильтрации и перекрестном выделении см. в статье [Фильтры и выделение в отчетах Power BI](../create-reports/power-bi-reports-filters-and-highlighting.md).

![Снимок экрана отчета в браузере со стрелкой к параметру "Фильтры".](media/end-user-report-filter/power-bi-reports.png)
...
![Снимок экрана отчета с раскрытой панелью "Фильтры".](media/end-user-report-filter/power-bi-expand-filters-pane.png)

Панель **Фильтры** содержит фильтры, которые были добавлены в отчет *разработчиком*. *Бизнес-пользователи*, такие как вы, могут взаимодействовать с имеющимися фильтрами и сохранять изменения, но не могут добавлять в отчет новые фильтры. Например, на снимке экрана выше конструктор добавил три фильтра уровня страницы: **сегмент — все**, **год — 2014**, а **регион — центральный**. Вы можете взаимодействовать с этими фильтрами и изменять их, однако добавить четвертый фильтр уровня страницы нельзя.

Некоторые фильтры затенены, а некоторые нет. Затенение фильтра означает, что он применен и некоторые данные исключаются. Например, карточка фильтра **Регион** затенена, и когда вы развернете ее, то увидите, что в раскрывающемся списке выбран только регион **Центральный**. Так как параметр "Регион" находится под заголовком **Фильтры на этой странице**, все визуальные элементы на этой странице не отображают (исключая) данные для регионов **Западный** и **Восточный**.

![Снимок экрана: развернутый фильтр "Регион" с выбранным пунктом "Центральный".](media/end-user-report-filter/power-bi-filter-region.png)
...
![Снимок экрана отчета с выделенными визуализацией и связанными с ней фильтрами.](media/end-user-report-filter/power-bi-filter-pane.png)
...
![Снимок экрана: список фильтров со стрелками, указывающими на эти фильтры в панели "Фильтры".](media/end-user-report-filter/power-bi-filters-hover.png)
...
В этом примере:
1. Фильтр **Включено** сообщает нам о перекрестной фильтрации визуального элемента. Это означает, что штаты Алабама и Техас были выбраны на одном из визуальных элементов на этой странице отчетов. В этом случае, это визуальный элемент карты. В результате выбора этих двух штатов были удалены данные для всех остальных штатов, отображаемых на выбранной линейчатой диаграмме.  
...
![Снимок экрана поиска в фильтре.](media/end-user-report-filter/power-bi-search-filter.png)
...
![Снимок экрана с фильтром, показывающий выбор западного региона.](media/end-user-report-filter/power-bi-filters-expand.png)
...
![Снимок экрана отчета и фильтров с выбранным визуальным элементом диаграммы "Дерево".](media/end-user-report-filter/power-bi-treemap-filter.png)
...
![Снимок значка ластика.](media/end-user-report-filter/power-bi-erase.png)
