...
---
# <a name="use-composite-models-in-power-bi-desktop-preview"></a>Использование составных моделей в Power BI Desktop (предварительная версия)
...
* **Составные модели** позволяют включить в отчет несколько подключений к данным, в том числе подключения DirectQuery или импорт, в любой комбинации. В этой статье приведено подробное описание составных моделей.
...
## <a name="enable-the-composite-models-preview-feature"></a>Включение функции предварительной версии "Составные модели"

Функция составных моделей находится в режиме предварительной версии, поэтому ее необходимо сначала включить в Power BI Desktop. Чтобы включить составные модели, последовательно выберите пункты **Файл** > **Параметры и настройки** > **Параметры** > **Функции предварительной версии** и установите флажок **Составные модели**. 
...
В следующем примере показан распространенный вариант таблицы *измерения* &mdash; такой как *Продукт* или *Клиент*, &mdash; которая расширяется с помощью некоторых дополнительных данных, импортированных из другого источника. Кроме того, таблицы могут использовать DirectQuery для подключения к различным источникам. Продолжая наш пример, представим, что данные о целях продаж (*Sales Targets*) по странам (*Country*) и периодам (*Period*) хранятся в отдельной базе данных отдела. Как обычно, подключиться к этим данным можно с помощью функции *GetData*, как показано на приведенном ниже рисунке: 
