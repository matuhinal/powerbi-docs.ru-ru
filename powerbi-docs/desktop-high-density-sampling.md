...
Начиная с выпуска **Power BI Desktop** за июнь 2017 года и обновлений **службы Power BI** доступен новый алгоритм выборки, который улучшает визуальные элементы, использующие выборку данных высокой плотности. Например, можно создать график результатов продаж розничных магазинов, выписывающих более десяти тысяч товарных чеков ежегодно. Графика таких сведений о продажах бы выборку (выбирать информативное представление данных, чтобы показать, как изменяются продажи со временем) из данных для каждого магазина и создать смешанный график, тем самым представляет базовые данные. Это распространенный подход к визуализации данных с высокой плотностью. В Power BI Desktop улучшена выборка данных с высокой плотностью. Этому и посвящена статья.
...
## <a name="how-high-density-line-sampling-works"></a>Выборка работает как с высокой плотностью линий
Ранее в **Power BI** детерминированным образом отбирался набор выборочных точек данных на полном диапазоне базовых данных. Например, для данных высокой плотности визуального элемента за один календарный год может отображаться 350 выборочных точек данных, которые были выбраны, чтобы в визуальном элементе был представлен полный диапазон данных (весь ряд базовых данных). Чтобы понять, как это происходит, представьте построение графиков цены акций за год период, а также выбираются 365 точек данных для создания графика visual (по одной точке данных на каждый день).
...
Для визуального элемента с высокой плотностью **Power BI** интеллектуально разделяет данные на блоки с высоким разрешением, а затем выбирает важные точки для представления каждого блока. Что процесс создания срезов данных с высоким разрешением специально настроены так, чтобы убедиться, что полученная диаграмма визуально неотличим от все базовые точки данных, но гораздо быстрее и интерактивнее.

### <a name="minimum-and-maximum-values-for-high-density-line-visuals"></a>Минимальное и максимальное значения для визуальных элементов линий с высокой плотностью
...
### <a name="evaluating-representative-data-points-for-high-density-line-visuals"></a>Оценка репрезентативных точек данных для визуальных элементов линий с высокой плотностью
...
## <a name="tooltips-and-high-density-line-sampling"></a>Всплывающие подсказки и выборка линий с высокой плотностью
...
Предположим, вы создаете визуальный элемент на основе цены акций и сравниваете две различные акции, которые используете **High-Density выборки**. Базовые данные для каждого ряда содержат большое количество точек данных (например, вы могли записывать цены акций за каждую секунду). Алгоритм выборки линий с высокой плотностью позволяет независимо выполнять группирование каждого ряда.
...
## <a name="how-to-turn-on-high-density-line-sampling"></a>Включение линий с высокой плотностью
По умолчанию новый алгоритм **включен**. Чтобы изменить этот параметр, перейдите к **форматирование** области в **Общие** карточки и внизу, вы увидите переключатель **High-Density выборки**. Чтобы отключить алгоритм, переключите его в положение **Выключено**.
...
* Из-за повышенной степени детализации и процесса группирования **подсказки** могут содержать значение, только если указатель мыши наведен на репрезентативные данные. См. в разделе *всплывающие подсказки и выборка линий с высокой плотностью* данной статьи, Дополнительные сведения.
