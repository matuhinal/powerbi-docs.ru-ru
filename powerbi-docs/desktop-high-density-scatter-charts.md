---
title: Высокая плотность точечной диаграммы в Power BI
description: Высокая плотность точечной диаграммы в Power BI
...
При включении **High-Density выборки**, Power BI реализует алгоритм, который устраняет перекрывающиеся точки и гарантирует, что при взаимодействии с ним можно связаться точек на визуальный элемент. Этот алгоритм также гарантирует, что все точки в наборе данных представлены в визуальном элементе. Он обеспечивает контекст для значения выбранных точек, а не просто отображает репрезентативную выборку.
...
Существуют обстоятельства, при которых **High-Density выборки** не может использоваться для точечной диаграммы и исходный алгоритм. Этот происходит при следующих обстоятельствах:
...
Чтобы включить **High-Density выборки**, выберите в точечную диаграмму, перейдите в **форматирование** области разверните **Общие** карт и в нижней части этой карты, передвиньте **Выборки high-Density** положение **на**.
...
* **High-Density выборки** алгоритм работает только с активными подключениями к моделям на основе службы Power BI, импортированным моделям и DirectQuery.
