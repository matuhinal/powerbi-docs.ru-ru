...
Для простых аддитивных мер, как *Общий объем продаж* в этом примере, сравнение основывается на относительных, а не абсолютных значениях. Таким образом несмотря на продажи для Touring Bikes ниже, чем общая продаж для всех категорий, по умолчанию визуальный элемент использует два оси позволяет сравнение долю продаж в разных странах на туристические велосипеды, и все категории bikes.  С помощью переключателя под визуальным элементом можно отобразить два значения на одной оси, чтобы вам было проще проводить сравнение с абсолютными значениями (как показано на следующем рисунке).    
...
Вы скорее всего спросить, что *различных* означает. Предположим, что общий объем продаж между США и Канадой распределяется следующим образом.
...
Для аддитивных мер, таких как *объем продаж* в предыдущем примере, используются гистограммы и графики, где две оси с соответствующим масштабированием позволяют легко сравнивать значения. Столбцы показывают значения с применением фильтра, а линии— общие значения (ось столбцов находится слева, а ось строк — справа, как обычно). Строки приводится с использованием *ступенчатый* стиле, используя пунктирной линией, заполненный серый цвет. Для предыдущего примера Если максимальное значение оси столбцов равно 4, а максимальное значение оси строки равна 20, затем она позволяет легко сравнить относительные значения между США и Канады для отфильтрованных и общее значение. 
...
После изучения различных столбцов и их значений выбирается набор значений с самыми большими отличиями. Для простоты понимания затем они выводятся, сгруппированные по столбцу, и первым указывается столбец, значения в котором отличаются больше всего. Показаны до трех значений каждого столбца, но меньше может отображаться при наличии менее чем из трех значений, которые сильно влиять либо если некоторые значения являются гораздо более эффективные, чем другие. 
