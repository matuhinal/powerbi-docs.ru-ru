...
Если в визуальный элемент также добавлен город (*City*), при известной численности населения по городам (*City*) отображаемый объем продаж (*Sales*) для поля *City* просто повторит число *Sales* для соответствующего штата (*State*) (как это обычно бывает при группировании по столбцу, не связанному с каким-либо статистическим показателем), как показано на следующем рисунке.
