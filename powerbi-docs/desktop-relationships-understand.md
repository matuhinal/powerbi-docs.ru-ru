...
Если для направления кроссфильтрации установлено значение **Оба**, доступно дополнительное свойство для двунаправленной фильтрации, когда принудительно применяются правила безопасности на уровне строк (RLS). Дополнительные сведения о RLS см. в статье [Безопасность на уровне строк (RLS) в Power BI Desktop](desktop-rls.md).
...
Рекомендуется использовать двунаправленную фильтрацию только при необходимости.<!-- For guidance on bi-directional filtering, see the [Cross filter relationship guidance](guidance/relationships-bidirectional-filtering) article.-->
...
Это свойство всегда должно быть включено, если между двумя таблицами существует ограничение внешнего ключа базы данных. Если такого ограничения нет, все равно можно включить свойство при условии наличия определенной целостности данных.
...
- Видео. [Рекомендации и запреты при использовании связей в Power BI](https://youtu.be/78d6mwR8GtA)
