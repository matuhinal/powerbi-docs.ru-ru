...
Предположим, Анна (бизнес-аналитик) работает в вашей команде. Она умеет создавать отличные модели данных (часто называемые наборами данных). Используя свой опыт, Анна может создать набор данных и отчет, а затем предоставить доступ к нему в **службе Power BI**.
...
Когда Анна создаст отчет (и создает набор данных, на котором он основан) и опубликует его в **службе Power BI**, он отобразится в рабочей области ее команды в службе Power BI. Если она сохраняет его в *новом интерфейсе рабочих областей*, то может задать разрешение "Сборка", чтобы все пользователи в ее рабочей области и вне ее могли просматривать и использовать этот отчет.

Дополнительные сведения о новом интерфейсе рабочих областей см. в статье [Рабочие области приложений](service-new-workspaces.md).

Другие участники в этой рабочей области и вне ее теперь смогут установить динамическое подключение к общей модели данных Анны, используя функцию **динамического подключения в службе Power BI**, и создавать собственные уникальные отчеты на основе *ее исходного набора данных* в *своих рабочих областях с новым интерфейсом*.
...
Теперь, когда ваш отчет с его набором данных, находится в службе Power BI, вы также можете *повысить его уровень*, чтобы подтвердить его качество и надежность. Вы даже можете запросить, чтобы он был *сертифицирован* центром в вашем клиенте Power BI. При наличии любого из таких подтверждений набор данных всегда будет отображаться в начале списка, когда люди ищут наборы данных. Если вы заинтересовались, можете ознакомиться с дополнительными сведениями о процессе [повышения уровня вашего набора данных](service-datasets-promote.md). 

Последним шагом является установка *разрешения "Сборка"* для набора данных, на котором основан отчет. Разрешение "Сборка" определяет, кто может видеть и использовать набор данных. Вы можете задать его в самой рабочей области либо при предоставлении приложения для общего доступа из рабочей области. См. дополнительные сведения о настройке [разрешения "Сборка"](service-datasets-build-permissions.md#build-permissions-for-shared-datasets).
...
Чтобы установить подключение к опубликованному отчету и создать собственный отчет на основе опубликованного набора данных, выберите **Получить данные** на ленте **Главная** в **Power BI Desktop**, **Power BI** в левой области и затем **Наборы данных Power BI**.
...
* Так как это динамическое подключение, навигация слева и моделирование отключены, как и при подключении к **SQL Server Analysis Services**, и вы можете подключиться только к одному набору данных в каждом отчете.
* Так как это динамическое подключение, применяются RLS (безопасность на уровне строк и ролей) и другое подобное поведение при подключении, как и при подключении к **SQL Server Analysis Services**.
