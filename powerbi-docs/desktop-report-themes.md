...
Кроме того, вы можете настроить и стандартизировать почти все элементы, отображаемые в области **Форматирование**, либо непосредственно в Power BI Desktop, либо с помощью JSON-файла темы. Цель — дать пользователю полный контроль над внешним видом отчета вплоть до мелочей.

## <a name="how-report-themes-work"></a>Как работают темы отчета
Чтобы применить тему отчета к отчету Power BI Desktop, можно выбрать одну из доступных встроенных тем отчета или создать либо импортировать пользовательскую тему.
...
Есть два способа настройки тем в Power BI Desktop. Рассмотрим каждый из них по очереди.
...
* создание и настройка темы в Power BI Desktop (предварительная версия);
* создание и настройка JSON-файла пользовательской темы отчета.

Чтобы настроить тему непосредственно в Power BI Desktop, необходимо сначала выбрать **Файл > Параметры и настройки > Параметры**, а затем в разделе **Предварительные версии возможностей** установите флажок **Настроить текущую тему**, как показано на рисунке ниже.

![Включение настраиваемых тем](media/desktop-report-themes/report-themes_5a.png)

Чтобы включить предварительную версию функции, вам может быть предложено перезапустить Power BI Desktop.

После перезапуска можно начать настройку текущей темы, выбрав вкладку **Главная страница** ленты, а затем выбрав команду **Переключить тему > Настроить текущую тему**. Откроется диалоговое окно со множеством параметров для настройки существующей темы.

![Настройка темы](media/desktop-report-themes/report-themes_5b.png)

Если вам нравится одна из существующих тем и вы хотите внести в нее лишь несколько изменений, выберите ее, а затем в диалоговом окне нажмите **Настроить текущую тему**, как показано на рисунке ниже. 

![Настройка текущей темы](media/desktop-report-themes/report-themes_5c.png)

> [!NOTE]
> Предыдущий снимок экрана был получен с включенной новой лентой, которая в настоящее время доступна в предварительной версии. Чтобы включить предварительную версию новой ленты, выберите **Файл > Параметры и настройки > Параметры**, а затем в разделе **Предварительные версии возможностей** установите флажок **Предварительная версия новой ленты**.

Параметры темы, которые можно настроить, разделены в диалоговом окне настройки темы на следующие категории:

* название темы (настраиваемую тему нужно как-либо назвать) и различные параметры цвета (цвета темы, цвета тональностей, цвета отклонения и т. д.);
* параметры текста, включая семейство, размер и цвет шрифта, а также заголовки осей, цвета, карточки, ключевые показатели эффективности и заголовки вкладок;
* компоненты визуальных элементов, такие как фон, граница, заголовок и подсказки;
* элементы страницы, такие как фоновый рисунок и фон;
* параметры области фильтров, в том числе цвет фона, прозрачность, цвет шрифта и значка, размер, карточки фильтров и многое другое.

После внесения изменений и нажатия кнопки **Применить и сохранить** ваша тема сохранится, и ее можно будет использовать в текущем отчете или экспортировать. 

Таким образом можно легко, быстро и наглядно настраивать текущую тему. Однако есть ряд окончательных изменений, которые требуют изменения JSON файла темы, как описано в следующем разделе.

> [!TIP]
> Большинство элементов темы можно настроить с помощью визуальных элементов в диалоговом окне **Настроить текущую тему**, после чего JSON-файл можно экспортировать и произвести детальную настройку вручную (изменив сам JSON-файл). Затем можно переименовать измененный JSON-файл, импортировать его и получить все необходимые настройки.
