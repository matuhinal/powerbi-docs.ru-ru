...
Начиная с выпуска **Power BI Desktop** за июнь 2018 года вы можете использовать соединитель SAP BW, в котором реализованы значительные улучшения в плане производительности и возможностей. Эта обновленная версия SAP BW была разработана корпорацией Майкрософт и называется **реализация 2.0**. Вы можете выбрать стандартный **соединитель SAP BW** или **реализацию 2.0 соединителя SAP**. В следующих разделах по очереди описывается установка каждой версии. При подключении к SAP BW из Power BI Desktop можно выбрать один из соединителей.

Мы рекомендуем по возможности использовать **реализацию 2.0 соединителя SAP**.

## <a name="installation-of-the-standard-sap-bw-connector"></a>Установка стандартного соединителя SAP BW
Мы рекомендуем по возможности использовать реализацию 2.0 соединителя SAP (см. инструкции в следующем разделе). В этом разделе описывается установка стандартного **соединителя SAP BW**, который можно установить, выполнив указанные ниже действия.
...
> [!NOTE]
> Стандартная реализация SAP BW требовала библиотек DLL Netweaver. Если вы используете реализацию 2.0 соединителя SAP и не используете стандартную версию, библиотеки DLL Netweaver не нужны.


## <a name="standard-sap-bw-connector-features"></a>Возможности стандартного соединителя с SAP BW
Стандартный **соединитель с SAP BW** в Power BI Desktop позволяет импортировать данные из кубов **сервера SAP Business Warehouse** или использовать DirectQuery. 
...
3. Нажмите кнопку **ОК**. Дальнейшие действия в окне **Навигатор** аналогичны описанным в предыдущем разделе для стандартного соединителя с SAP BW. 
...
4. Мы приложили все усилия, чтобы сделать реализацию 2.0 соединителя с SAP BW совместимой со стандартным соединителем с SAP BW. Однако могут быть некоторые различия из-за использования разных режимов выполнения многомерных выражений SAP BW. Чтобы устранить несоответствия, попробуйте сменить режим выполнения.
