...
description: Использование режима хранения для контроля кэширования данных в памяти для отчетов в Power BI Desktop
...
* **Связи "многие ко многим"** — с помощью **составных моделей** можно устанавливать **связи "многие ко многим"** между таблицами, устраняя требования к уникальным значениям в таблицах и необходимость в прежних решениях, таких как введение новых таблиц только для установления связей. 
...
* **Режим хранения** подробно описан в этой статье.
...
Применение таких параметров свойств режима хранения приводит к следующему поведению, если предположить, что в таблице *Sales* содержится значительный объем данных.
...
**Двойной** режим хранения — это способ оптимизировать производительность. Его следует использовать так, чтобы это не затронуло возможность удовлетворения бизнес-требований. Для реализации альтернативного поведения рекомендуется использовать приемы, описанные в статье [Связи "многие ко многим" в Power BI Desktop (предварительная версия)](desktop-many-to-many-relationships.md).
