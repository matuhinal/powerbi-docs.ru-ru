...
Узнайте, как интегрировать (внедрить) панель мониторинга, плитку или отчет в веб-приложение с помощью пакета SDK для .NET в Power BI и API JavaScript для Power BI для клиентов. Как правило, это сценарий независимых поставщиков программного обеспечения.
...
* Government Community Cloud (GCC):
    ...
    2. Обновите поля clientid (идентификатор клиента собственного приложения), groupid, user (ваш главный пользователь) и password в файле Web.config.
...
Чтобы осуществлять вызовы REST API, необходимо зарегистрировать приложение в Azure AD. Дополнительные сведения см. в статье [Регистрация приложения Azure AD для внедрения содержимого Power BI](register-app.md). Так как национальные облака принадлежат различным организациям, существуют отдельные URL-адреса для регистрации приложений.
...
* Power BI для облачных служб в Китае — https://login.microsoftonline.cn
...
Используя маркер доступа, можно создать объект клиента Power BI, что позволит вам работать с API Power BI. Для этого AccessToken упаковывается в объект *Microsoft.Rest.TokenCredentials*.
...
Необходимо, чтобы создаваемый токен внедрения можно было использовать из API JavaScript. Токен внедрения связан только с внедряемым элементом. Это означает, что при каждом внедрении части содержимого Power BI нужно создавать отдельный токен внедрения. Дополнительные сведения, включая информацию о том, какой уровень **accessLevel** нужно использовать, см. в статье [Токен внедрения](https://docs.microsoft.com/rest/api/power-bi/embedtoken).
