---
title: Внедрение содержимого Power BI в приложение для клиентов для использования в правительственных и национальных облаках
...
Узнайте, как внедрить панель мониторинга, плитку или отчет в веб-приложение с помощью пакета SDK Power BI для .NET и API-интерфейса JavaScript для Power BI, реализуя интеграцию данных для клиентов.
...
Если вы скачали [пример внедрения для клиента](https://github.com/Microsoft/PowerBI-Developer-Samples/tree/master/App%20Owns%20Data), используйте полученный после регистрации идентификатор приложения **applicationId** для аутентификации этого примера приложения в Azure Active Directory. Для настройки примера измените значение **applicationId** в файле *web.config*.
