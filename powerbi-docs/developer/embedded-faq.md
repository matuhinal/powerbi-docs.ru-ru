...
### <a name="is-there-a-limit-to-the-number-of-embed-tokens-a-power-bi-master-account-can-generate"></a>Ограничено ли число токенов внедрения, которые можно создать в основной учетной записи Power BI? 

Да, ограничено, так как эти токены предназначены для только для разработки и тестирования. Для сценариев внедрения в рабочей среде [необходимо приобрести емкость](#technical). В этом случае количество создаваемых токенов внедрения не ограничено. Выберите [Получить доступные функции](https://msdn.microsoft.com/en-us/library/mt846473.aspx), чтобы проверить использованное число свободных токенов внедрения.
