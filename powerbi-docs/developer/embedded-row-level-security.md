...
Безопасность на уровне строк (RLS) может использоваться для ограничения доступа пользователей к данным внутри отчета или набора данных, позволяя нескольким различным пользователям использовать один и тот же отчет при просмотре различных данных. Безопасность на уровне строк может использоваться при внедрении отчетов из Power BI.

Если вы выполняете внедрение для пользователей, не работающих с Power BI (данные принадлежат приложению), что обычно является сценарием ISV, то эта статья предназначена именно для вас. Вам понадобится настроить маркер внедрения для учетной записи пользователя и роли. Далее вы узнаете, как это сделать.
...
**Пользователи.** Это пользователи, просматривающие отчеты. В Power BI Embedded пользователи идентифицируются по свойству имени пользователя в маркере внедрения.
...
API [GenerateToken](https://msdn.microsoft.com/library/mt784614.aspx) принимает список удостоверений с обозначением соответствующих наборов данных. Сейчас можно указать только одно удостоверение. В будущем будет добавлена поддержка нескольких наборов данных для внедрения информационных панелей. Для работы RLS необходимо передать следующее как часть удостоверения.
...
* **dataset (обязательно).** Это набор данных, предназначенный для внедряемого отчета. В списке наборов данных можно указать только один набор. В будущем будет поддерживаться несколько наборов данных для внедрения информационных панелей.
...
Теперь, когда все части соединены, когда кто-то войдет в ваше приложение для просмотра этого отчета, будут отображаться только данные, на просмотр которых у пользователя есть разрешение, как это определено безопасностью на уровне строк.
...
* Передача информации об удостоверении при вызове GenerateToken поддерживается только для чтения и записи отчета. Поддержка других ресурсов будет доступна позже.
...
* Если базовый набор данных является облачной моделью (кэшированная модель или DirectQuery), действующее удостоверение должно включать хотя бы одну роль. В противном случае роли не буду назначены.
* В списке удостоверений можно указать только одно удостоверение. Мы используем список для включения маркеров с несколькими удостоверениями для внедрения информационных панелей в будущем.
