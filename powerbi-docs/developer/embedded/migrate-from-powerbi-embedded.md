...
1. Убедитесь в наличии доступа к клиенту Azure Active Directory (Azure AD).
   ...
   * Использовать отдельный клиент для каждого пользователя?
     ...
     Если вы решили создать клиент для приложения или для каждого пользователя, ознакомьтесь с разделом [Создание клиента Azure Active Directory](create-an-azure-active-directory-tenant.md) или [Получение клиента Azure Active Directory](/azure/active-directory/develop/active-directory-howto-tenant).
...
2. Перестройте параметры проверки подлинности AuthN и AuthZ с помощью *главной* учетной записи приложения. Вы можете воспользоваться преимуществами [маркера внедрения](/rest/api/power-bi/embedtoken), чтобы разрешить этому пользователю действовать от имени других пользователей.
...
* Приобретите необходимый размер емкости. Чтобы определиться, какой объем и тип вам нужны, см. [технический документ по планированию ресурсов аналитики Power BI Embedded](./embedded-capacity-planning.md). Вы можете [приобрести емкость](https://portal.azure.com/#create/Microsoft.PowerBIDedicated) в Azure.
...
[Технический документ по планированию ресурсов аналитики коллекции рабочих областей](./embedded-capacity-planning.md)  
