...
Ниже приведены действия, которые позволяют реализовать эту возможность. Мы рассмотрим, что необходимо сделать, чтобы создать и использовать внедренное содержимое в приложении.
...
Прежде чем начать внедрение панелей мониторинга и отчетов в приложение, необходимо убедиться в том, что ваша среда настроена для внедрения. В процессе настройки необходимо выполнить указанные ниже действия.
...
* [Регистрация приложения и разрешения](embedding-content.md#appreg)
* [Создание рабочих областей приложений](embedding-content.md#appws)
* [Создание и передача отчетов](embedding-content.md#createreports)
...
> [!NOTE]
> Выделенная емкость не является обязательным условием для разработки приложения. Разработчикам приложения требуется лицензия Power BI Pro.

### <a name="azureadtenant"></a>Клиент Azure Active Directory

Чтобы внедрить элементы из Power BI, требуется клиент Azure Active Directory (Azure AD). У этого клиента должен быть по крайней мере один пользователь Power BI Pro. Кроме того, необходимо определить приложение Azure AD в данном клиенте. Можно использовать существующий клиент Azure AD или создать новый клиент специально для внедрения.
...
Не советуем использовать для приложения учетную запись глобального администратора организации или клиента, если выполняется внедрение для клиентов. Так вы ограничите доступ для учетной записи приложения в клиенте. Необходимо, чтобы администратор управлял всеми рабочими областями приложений, созданными для внедрения.

#### <a name="accounts-for-analysts-that-create-content"></a>Учетные записи аналитиков, создающих содержимое

У вас может быть несколько пользователей, которые создают содержимое для Power BI. Вам необходима учетная запись Power BI Pro для каждого аналитика, который создает и развертывает содержимое в Power BI.
...
Чтобы осуществлять вызовы REST API, необходимо зарегистрировать приложение в Azure AD. Дополнительные сведения см. в статье [Регистрация приложения Azure AD для внедрения содержимого Power BI](register-app.md).

### <a name="appws"></a>Создание рабочих областей приложений
...
### <a name="createreports"></a>Создание и передача отчетов
...
В приложении необходимо пройти проверку подлинности в Power BI. Если вы внедряете содержимое для клиентов, учетные данные *главной* учетной записи сохраняются в приложении.

> [!NOTE]
> Дополнительные сведения о проверке подлинности пользователей при внедрении для клиентов см. в разделе [Проверка подлинности для пользователей и получение маркера доступа Azure AD для приложения Power BI](get-azuread-access-token.md).
>
...
Сведения о **внедрении для организации** см. в следующем руководстве:
...
* [Руководство. Внедрение отчета, панели мониторинга или плитки Power BI в приложение для клиентов](embed-sample-for-customers.md)

При внедрении содержимого для клиентов требуется токен внедрения. Дополнительные сведения см. в статье [Токен внедрения](https://docs.microsoft.com/rest/api/power-bi/embedtoken).
...
Все пользователи, независимо от назначенной лицензии, могут использовать содержимое, которое внедрено из рабочей области приложения (группы), если эта рабочая область поддерживается выделенной емкостью. Таким образом, необходимо явно добавить в рабочую область приложения всех пользователей, у которых нет лицензии Power BI Pro. В противном случае появляется сообщение об ошибке 401 "Не санкционировано". В таблице ниже перечислены доступные номера SKU Power BI Premium в Office 365.
...
> [!NOTE]
> Чтобы приобрести Power BI Premium, необходимо быть глобальным администратором или администратором выставления счетов в клиенте. Сведения о том, как приобрести Power BI Premium, см. в разделе [Как купить Power BI Premium](../service-admin-premium-purchase.md).

>[!Note]
>[Настройте среду встроенной аналитики для организации.](#step-1-setup-your-embedded-analytics-development-environment)
>
...
* Измените рабочую область приложения и назначьте для нее выделенную емкость в диалоговом окне "Дополнительно".
...
>[!Note]
>[Настройте среду встроенной аналитики для клиентов.](#step-1-setup-your-embedded-analytics-development-environment) 
>
