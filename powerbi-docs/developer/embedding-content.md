...
Если вы внедряете панели мониторинга и отчеты для клиентов, эти компоненты должны быть размещены в рабочей области приложения. Сведения о том, как создать рабочую область приложения, см. в разделе [Создание рабочей области приложения](../service-create-distribute-apps.md#create-an-app-workspace).
...
Если вы внедряете содержимое для клиентов, сделайте следующее:
...
| Узел емкости | Общее число ядер<br/>*(серверная часть и интерфейс)* | Внутренние ядра | Интерфейсные ядра | Ограничения для подключений DirectQuery и активных подключений | Максимальное число страниц, отображаемых в часы максимальной нагрузки |
| --- | --- | --- | --- | --- | --- |
| A1 |1 виртуальное ядро |0,5 ядра, 3 ГБ ОЗУ |0,5 ядра | |1–300 |
| A2 |2 виртуальных ядра |1 ядро, 5 ГБ ОЗУ |1 ядро | |301–600 |
| A3 |4 виртуальных ядра |2 ядра, 10 ГБ ОЗУ |2 ядра | |601–1200 |
