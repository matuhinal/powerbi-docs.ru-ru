...
Разработчики отправляют в Microsoft AppSource свои визуальные элементы Power BI с функциями IAP через панель продавца (так же, как и бесплатные визуальные элементы). Чтобы указать, что отправляемый визуальный элемент Power BI включает функции IAP, разработчик должен записать в заметках панели продавца фразу "Визуальный элемент с покупкой из приложения". Кроме того, разработчик должен предоставить лицензионный ключ или токен, чтобы команда проверки могла проверить функции IAP. Когда визуальный элемент Power BI будет проверен и утвержден, в его листинге Microsoft AppSource в разделе цен будет указано "Может потребоваться дополнительная покупка".
