---
title: "Публикация пользовательских визуализаций в Магазине Office"
description: "Узнайте, как публиковать пользовательские визуализации в Магазине Office, чтобы они стали доступными для других пользователей."
services: powerbi
documentationcenter: 
author: guyinacube
manager: kfile
...
Создав пользовательскую визуализацию, опубликуйте ее в Магазине Office. Там ее смогут найти и скачать для своих целей другие пользователи. Перед публикацией необходимо выполнить некоторые подготовительные действия. Дополнительные сведения о том, как создать пользовательские визуализации, см. в статье [Создание пользовательского визуального элемента с помощью средств разработчика](../service-custom-visuals-getting-started-with-developer-tools.md).
...
| Значок |Да |Добавьте логотип пользовательской визуализации. Он будет отображаться в онлайн-магазине. Допустимые форматы: PNG, JPG, JPEG или GIF. Изображение должно быть ровно 300 пикселей в ширину и 300 пикселей в высоту. Размер файла не должен превышать 512 КБ. |
...
| Ссылка на видео |Нет |Чтобы повысить интерес пользователей к визуализации, рекомендуем указать ссылку на видео о вашем продукте. URL-адрес должен начинаться с https:// или http://. |
...
> [!NOTE]
> После того как визуализация пройдет процедуру утверждения в Магазине, все ее последующие обновления будут появляться в рабочей среде через две недели. Это сделано для повышения качества распространяемых материалов и проверки на совместимость с существующими отчетами.
> 
> 

## <a name="submitting-to-the-office-store"></a>Отправка в Магазин Office
Получив XML-файл с пакетом приложения от команды Power BI, перейдите в [Центр разработчика Office](https://sellerdashboard.microsoft.com/Application/Summary), чтобы отправить визуальный элемент в Магазин Office.
...
> 
> [!IMPORTANT]
> Перед отправкой визуализации в Магазин Office файлы PBIVIZ и PBIX необходимо отправить по электронной почте команде Power BI. Команда Power BI передаст эти файлы на общедоступный сервер общих ресурсов. Другого способа передать эти файлы в Магазин Office нет. Эти файлы нужно отправлять команде каждый раз, когда вы хотите передать в Магазин Office новую визуализацию, а также обновление для существующей или исправления для отклоненной визуализации.
> 
> 
...
16. Сейчас поддерживаются только *бесплатные* визуальные элементы, и возможность покупки в приложении (в визуальном элементе) отключена. Установите флажок **Это приложение бесплатно**. 
    ...
    > 
    > 
17. Теперь можно щелкнуть **"Сохранить как черновик", чтобы отправить пользовательский визуальный элемент позже, или **Отправить на утверждение**, чтобы отправить его в Магазин Office.
