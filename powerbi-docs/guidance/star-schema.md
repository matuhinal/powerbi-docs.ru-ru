...
В модели Power BI **мера** имеет другое, но схожее определение. Это формула, написанная на языке [выражений анализа данных (DAX)](/dax/data-analysis-expressions-dax-reference), которая служит для обобщения. В выражениях мер часто используются статистические функции DAX, такие как SUM, MIN, MAX, AVERAGE и т. д., для получения скалярного результата во время выполнения запроса (значения никогда не хранятся в модели). Выражением меры может быть как простое агрегирование столбцов, так и более сложная формула, переопределяющая контекст фильтра и распространение связей. Дополнительные сведения см. в статье [Основные сведения о DAX в Power BI Desktop](../transform-model/desktop-quickstart-learn-dax-basics.md).
...
- Если известно, что авторы отчетов будут запрашивать модель с помощью [многомерных выражений](/sql/analysis-services/multidimensional-models/mdx/mdx-query-the-basic-query), модель должна содержать _явные меры_. Явные меры определяются с помощью DAX. Такой подход особенно актуален, если запросы к набору данных Power BI выполняются с помощью многомерных выражений, так как многомерные выражения не позволяют обобщать значения столбцов. В частности, многомерные выражения будут использоваться при выполнении [анализа в Excel](../collaborate-share/service-analyze-in-excel.md), потому что сводные таблицы выдают запросы многомерных выражений.
...
Единственный способ использовать неактивную связь — определить выражение DAX, в котором применяется [функция USERELATIONSHIP](/dax/userelationship-function-dax). В нашем примере разработчик модели должен создать меры, чтобы обеспечить анализ продаж через торговых посредников по датам отгрузки и доставки. Это может потребовать много усилий, особенно если в таблице торговых посредников определено большое количество мер. Кроме того, из-за большого количества мер в области **Поля** работать может быть неудобно. Есть и другие ограничения.
...
Чтобы преодолеть эти ограничения, в Power BI обычно создается таблица измерения для каждого ролевого экземпляра. Как правило, дополнительные таблицы измерений создаются как [вычисляемые таблицы](/dax/calculatetable-function-dax) с помощью DAX. При использовании вычисляемых таблиц модель может содержать таблицу **Дата**, таблицу **Дата отгрузки** и таблицу **Дата доставки**, каждая из которых имеет одну активную связь с соответствующим столбцом таблицы продаж через торговых посредников.
...
Однако если в таблице продаж торговых посредников Adventure Works есть столбцы с номерами заказов _и_ номерами позиций заказов и эти столбцы необходимы для фильтрации, то таблица вырожденного измерения будет хорошим решением. Дополнительные сведения см. в [Руководство по связям типа "один к одному"](relationships-one-to-one.md#degenerate-dimensions).
...
Более полезным применением таблицы фактов без метрик является хранение связей между измерениями. Именно такой подход рекомендуется использовать в Power BI для определения связей "многие ко многим" между измерениями. В [структуре связей "многие ко многим" между измерениями](relationships-many-to-many.md#relate-many-to-many-dimensions) таблица фактов без метрик называется _сопоставительной таблицей_.
...
- [Создание связей и управление ими в Power BI Desktop](../transform-model/desktop-create-and-manage-relationships.md)
