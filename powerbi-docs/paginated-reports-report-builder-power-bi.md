...
## <a name="create-a-subscription-to-your-report"></a>Создание подписки на отчет

Теперь можно настроить подписки по электронной почте для себя и других отчетов с разбиением на страницы в службе Power BI. Как правило выполняется так же, как подписки на отчеты и панели мониторинга в службе Power BI. При настройке подписок, выберите, как часто вы хотите получать сообщения электронной почты: ежедневно, еженедельно или почасовой. Подписка содержит вложения в формате PDF выходных данных всего отчета.

Дополнительные сведения см. в статье [подписываться на отчеты с разбивкой на страницы в службе Power BI себя и других](paginated-reports-subscriptions.md). 
