...
- **Сервер отчетов Power BI**
    - *Версия: 1.8.7485.35104 (сборка 15.0.1103.234), дата выпуска: 30 июня 2020 года*
        - Исправленные ошибки
            - Исправлена проблема в сценариях горизонтального увеличения масштаба, когда отчеты сразу не отражали изменения на сервере после отправки.
    - *Версия: 1.8.7468.41510 (сборка 15.0.1103.232), дата выпуска: 15 июня 2020 года*
        - Исправленные ошибки
            - Исправлена проблема, когда отчеты сразу не отражали изменения на сервере после отправки.
    ...
    - *Версия: 1.8.7450.37410 (сборка 15.0.1103.227), дата выпуска: 27 мая 2020 г.*
        ...
        - Исправленные ошибки
            - Устранены две проблемы, связанные с одинарными кавычками в именах папок и отчетов.
            - Устранена проблема, связанная с использованием горизонтальной полосы прокрутки в некоторых браузерах и функции просмотра записей.
            - Устранена проблема, при которой запланированное обновление во время открытия отчета иногда приводит к ошибкам схемы в базовой модели.
            - Устранена проблема, при которой альтернативный текст для экспорта в PDF неправильно кодируется для многобайтовых символов.
            - Устранена проблема, из-за которой в пользовательских приложениях, выполняющих LoadReport, необоснованно выводится ошибка TrustedHeader.
            - Устранена проблема, при которой высокая нагрузка, возникающая в связи с запланированным обновлением, может приводить к сбоям обновления.
            - Устранена проблема, когда отчеты сохраняются в неправильном месте, если имя отчета соответствует имени папки.
            - Устранены проблемы с вкладками в схеме документа.
            - Устранена проблема, при которой управляемые данными подписки завершаются сбоем при использовании запросов DAX.
            - Устранена проблема с доступом к URL-адресу, когда FindString не удается обнаруживать совпадения.
            - Устранена проблема, которая приводит к сбою внедренных источников данных при перемещении отчетов.
            - Устранена проблема, которая приводит к сбою запланированного обновления для некоторых источников данных.
            - Добавлена проверка расписания отчета для сокращения вероятности недопустимых запросов.


- **Power BI Desktop (оптимизировано для решения "Сервер отчетов Power BI")** .
   - *Версия: 2.81.5831.1181 (май 2020 г.), дата выпуска: 9 июня 2020 года*
        - Исправление ошибок
           -  Исправление для визуальных элементов из MarketPlace
   - *Версия: 2.81.5831.941 (май 2020 г.), дата выпуска: 27 мая 2020 г.* (новая сборка и новая версия)
        - Содержит изменения, необходимые для подключения к Серверу отчетов Power BI (май 2020 г.).        
