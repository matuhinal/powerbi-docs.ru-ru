...
- **Power BI Desktop (оптимизировано для Сервера отчетов Power BI)**
    ...
    - *Версия 1.2.6690.34729 (сборка 15.0.2.402), дата выпуска: 27 апреля 2018 г.*
        - Исправленные ошибки
            ...
            - Для отчетов с разбивкой на страницы (RDL):
                - Использование `Lookup()` или производных функций, например `LookupSet()` и `MultiLookup()`, в выражениях языка определения отчетов больше не вызывает `#Error`
...
- **Сервер отчетов Power BI**
    ...
    - *Версия 1.1.6514.9163 (сборка 14.0.600.434), дата выпуска: 1 ноября 2017 г.*
        - Исправленные ошибки
...
[Установить сервер отчетов Power BI](install-report-server.md)  
[Скачивание построителя отчетов](https://www.microsoft.com/download/details.aspx?id=53613)  
[Скачивание SQL Server Data Tools (SSDT)](http://go.microsoft.com/fwlink/?LinkID=616714)
