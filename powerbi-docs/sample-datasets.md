...
ms.locfileid: "61206630"
---
# <a name="what-sample-data-is-available-to-use-with-power-bi"></a>Примеры данных, доступные для использования в службе Power BI
Предположим, что вы не знакомы с Power BI и хотите оценить эту службу, но у вас нет данных.  Или, возможно, что у вас есть набор данных. Так как вы не понимаете (пока) принципов работы Power BI, пользователи опасаются, что вы боитесь повредить ваш набор данных?

Не переживайте! Корпорация Майкрософт и obviEnce ([www.obvience.com](http://www.obvience.com/)) создали примеры данных, которые можно использовать для изучения принципов работы в Power BI.  Эти данные были анонимизированы и взяты из разных отраслей, например финансы, персонал, продажи и многое другое. Кроме того нашей электронной документацией использует эти примеры, в руководствах и примерах, поэтому вы сможете продолжить работу.      

   ![Доступные примеры](media/sample-datasets/power-bi-samples.png)

Каждый из этих примеров доступен в нескольких форматах: как пакет содержимого, отдельная книга Excel и PBIX-файл. Если вы не знаете, что это такое или как их получить, не переживайте. Мы все подробно объясним далее в этой статье. Для каждого примера мы создали *Обзор*. Учебники, статьи, в которых содержатся общие сведения о примере и рассматриваются различные сценарии. Один сценарий может содержать полезные сведения для менеджеров, другой — сведения о выполнении конкурентного анализа, создании отчетов и информационных панелей для совместного использования. Кроме того, в них могут объясняться их преимущества для бизнеса.   

Прежде чем начать, ниже приведены правила и условия использования примеров. После этого мы о примерах и показано, как их использовать.   
...
Этом отраслевом примере анализируются ключевые показатели финансового Директора для руководителей, продуктов и клиентов компании. Можно выяснить, какие факторы влияют на рентабельность компании.
...
В этом примере рассматривается стратегия найма в компании путем анализа новых сотрудников, действующих сотрудников и сотрудников, которые покинули.  В процессе изучения данных можно определить тенденции добровольных уходов и дисбаланса в стратегии найма.
...
Как уже говорилось, примеры доступны в нескольких форматах: пакеты содержимого, книги Excel и PBIX-файлы. Мы объясним, как использовать каждый из этих форматов, начиная с пакетов содержимого.

## <a name="the-power-bi-samples-as-content-packs"></a>Примеры Power BI в виде пакетов содержимого
Пакеты содержимого — это единственный формат примеров, который можно получить из Power BI. Вам даже не нужно выходить из Power BI, чтобы найти их. Пакет содержимого — это по сути пакет, один или несколько панелей мониторинга, наборами данных и отчетов, кто-то создает и который может использоваться со службой Power BI. Пользователи создают пакеты содержимого, чтобы предоставить доступ к данным другим сотрудникам. Каждый из примеров пакетов содержимого Power BI содержит набор данных, отчет и информационную панель.  Они недоступны для Power BI Desktop. Дополнительные сведения о пакетах содержимого см. в статье [Знакомство с пакетами содержимого организации в Power BI](service-organizational-content-pack-introduction.md).
...
1. Откройте службу Power BI (app.powerbi.com) и выполните вход.
2. В нижнем левом углу, выберите **данные**.
...
2. Сохраните скачанный файл. Важно, где сохранен файл.
   ...
   * ![Local](media/sample-datasets/power-bi-local-file2.png) **Локально**. При сохранении файла на локальном диске на компьютере или в другом расположении в вашей организации вы можете импортировать файл в Power BI. На самом деле, файл останется на локальном диске, чтобы не импортировать весь файл в Power BI. Что произойдет, — новый набор данных создается на сайте Power BI и данные и в некоторых случаях модель данных, загружаются в набор данных. Если файл содержит все отчеты, эти отчеты отображаются в своем сайте Power BI в разделе отчеты.
   * ![OneDrive бизнеса](media/sample-datasets/power-bi-onedrive-file.png) **OneDrive для бизнеса** – Если у вас есть OneDrive для бизнеса, чтобы войти в него с помощью той же учетной записи, войдите в Power BI с OneDrive для бизнеса — несомненно, лучший способ синхронизировать вашу работу в Excel Power BI, или. CSV-ФАЙЛ в синхронизации файлов с помощью вашего набора данных, отчеты и панели мониторинга в Power BI. Так как Power BI и OneDrive находятся в облаке, Power BI подключается к вашему файлу в OneDrive примерно раз в час. При обнаружении каких-либо изменений ваш набор данных, отчеты и информационные панели в Power BI автоматически обновляются.
...
1. Откройте службу Power BI (app.powerbi.com) и выполните вход.
   ...
   >
   >
4. В нижнем левом углу, выберите **данные**.
...
7. Выберите файл, в этом случае **Procurement Analysis Sample.xlsx**, который был сохранен в OneDrive для бизнеса и выберите **Connect**.
...
10. Когда появится экран **Набор данных готов.** , Появится экран, выберите **просмотреть набор данных** или **получить краткую аналитику** или найдите и откройте связанный отчет или панель мониторинга с помощью левой навигационной панели Power BI.  
...
* В первый раз при открытии образца книги в Excel появляются два предупреждения. Первое предупреждение о том, что книга находится в режиме защищенного просмотра. Выберите **Включить редактирование**. Второе предупреждение о может сообщить, что книга содержит подключения к внешним данным. Выберите **Включить содержимое**.
* Каждая книга содержит несколько листов Power View. Если вы хотите увидеть листы Power View в Excel, необходимо [включить надстройку Power View](https://support.office.com/article/flash-silverlight-and-shockwave-controls-blocked-in-microsoft-office-55738f12-a01d-420e-a533-7cef1ff6aeb1) , загрузив пакет разделов реестра.
* Так где же фактические данные? Они находятся в модели данных Power Pivot. Листы Power View, чтобы просмотреть данные, не требуется. На **PowerPivot** выберите **управление**.

    Не видите **Power Pivot** вкладке? [Включите надстройку Power Pivot](https://support.office.com/article/Start-Power-Pivot-in-Microsoft-Excel-2013-add-in-A891A66D-36E3-43FC-81E8-FC4798F39EA8).

    В Power Pivot вы увидите данные в базовых таблицах, а также все формулы DAX. 
