...
Для аудита журналов Power BI зайдите в центр безопасности и соответствия требованиям Office 365.

После включения аудита его данные могут отобразиться с задержкой до 48 часов. Если данные не отображаются сразу, проверьте журналы аудита позже. Аналогичная задержка возможна при получении разрешения на просмотр журналов аудита: доступ к ним может быть предоставлен не сразу.
...
| Изменен состав пользователей источника данных шлюза Power BI |Это действие регистрируется при каждом изменении пользователей шлюза (добавление или удаление). |Имя шлюза, добавленные пользователи, удаленные пользователи |
