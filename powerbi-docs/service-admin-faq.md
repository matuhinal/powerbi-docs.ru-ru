...
* **Сценарий 3.** Если в вашей организации нет среды Office 365, подключенной к домену электронной почты, принцип управления удостоверениями не изменится.
...
Power BI предоставляет портал администрирования, который позволяет просматривать статистику использования, обеспечивает ссылки на Центр администрирования Office 365 для управления пользователями и группами, а также дает возможность управлять параметрами на уровне клиента.
...
Если не предпринимать никаких действий, клиент создается для каждого домена и поддомена электронной почты пользователя. Если требуется, чтобы все пользователи были в одном клиенте, независимо от расширения их адресов электронной почты, заранее создайте целевой клиент или используйте существующий и добавьте в него все домены и поддомены, которые необходимо объединить в этом клиенте. В этом случае все пользователи, адреса электронной почты которых заканчиваются этими доменами и поддоменами, автоматически добавятся к целевому клиенту после регистрации.
...
1. Перейдите в [Центр администрирования Office 365](https://admin.microsoft.com/AdminPortal/Home#/homepage).
