...
Все пользователи могут увидеть **портал администрирования** под значком с шестеренкой. Если у них нет административных полномочий, они могут увидеть только раздел **Параметры Premium** и те емкости, на управление которыми у них есть права.
...
После загрузки информационной панели вы можете увидеть два раздела плиток. Первый раздел содержит данные об использовании для отдельных пользователей, а второй — аналогичные сведения для групп в организации.
...
Во втором разделе показаны те же сведения, но для групп. Эти плитки позволяют вам понять, какие группы в организации являются наиболее активными и какие сведения они используют.
...
* **Отключено для всей организации**. Вы можете отключить функцию и сделать так, чтобы пользователи не могли использовать ее.

    ![](media/service-admin-portal/powerbi-admin-tenant-settings-disabled.png)

* **Включено для всей организации**. Вы можете включить функцию для всей организации, предоставив к ней доступ всем пользователям.

    ![](media/service-admin-portal/powerbi-admin-tenant-settings-enabled.png)

* **Включено для части организации**. Функцию также можно включить для определенной части организации. Это можно сделать несколькими способами. Вы можете включить ее для всей организации, за исключением определенной группы пользователей.
...
Пользователи в организации могут создавать рабочие области приложений для совместной работы над панелями мониторинга, отчетами и другим содержимым.

Дополнительные сведения см. в статье [Создание рабочих областей](service-create-the-new-workspaces.md).
...
Так выглядит сообщение, которое отображается при предоставлении общего доступа внешнему пользователю.
...
Пользователи могут увидеть разные параметры в пользовательском интерфейсе в зависимости от настроек параметра "Опубликовать в Интернете".
...
Пользователи в организации могут использовать визуализацию ArcGIS Maps for Power BI от Esri. [Дополнительные сведения](power-bi-visualization-arcgis.md)
...
Для сбора записей журнала аудита этот параметр необходимо включить. После включения аудита его данные могут отобразиться с задержкой до 48 часов. Если данные не отображаются сразу, проверьте журналы аудита позже. Аналогичная задержка возможна при получении разрешения на просмотр журналов аудита: доступ к ним может быть предоставлен не сразу.

> [!NOTE]
> Этот параметр применяется ко всей организации и не ограничивается определенными группами.

### <a name="usage-metrics-for-content-creators"></a>Метрики использования для создателей содержимого
Пользователи в организации могут просматривать метрики использования созданных информационных панелей и отчетов. [Дополнительные сведения](service-usage-metrics.md).

Вы можете **включить** этот параметр, а затем указать, кто может просматривать метрики использования (вся организация или отдельные группы безопасности).
...
Вы можете **включить** этот параметр, а затем указать, кто может просматривать отображаемые имена и адреса электронной почты в метриках использования (вся организация или отдельные группы безопасности).
...
Пользователи в организации могут внедрить панели мониторинга и отчеты Power BI в приложения SaaS. Если отключить этот параметр, пользователи не смогут использовать REST API для внедрения содержимого Power BI в приложение.

## <a name="capacity-settings"></a>Параметры емкости

### <a name="premium-settings"></a>Параметры категории "Премиум"

На вкладке параметров Premium можно управлять любыми емкостями Power BI Premium (номер SKU Em или P), приобретенными для вашей организации. Вкладка параметров Premium будет отображаться для всех пользователей вашей организации, но они смогут увидеть ее содержимое, только если им назначена роль **администратора емкости** или пользователя с назначенными разрешениями. Если у пользователей нет разрешений, появляется следующее сообщение.

![Параметры администрирования Power BI Premium](media/service-admin-portal/premium-settings-no-access.png "Отсутствие доступа к параметрам Premium")
...
### <a name="power-bi-embedded-settings"></a>Параметры Power BI Embedded

Вкладка параметров Power BI Embedded позволяет просматривать емкости Power BI Embedded, приобретенные для клиента. Так как вы можете приобрести только номер SKU A в Azure, вы можете [управлять емкостями Embedded в Azure](developer/azure-pbie-create-capacity.md) с **портала Azure**.

![Параметры администрирования Power BI Embedded](media/service-admin-portal/manage-pbie-capacities-01.png)

![Сведения о параметрах администрирования Power BI Embedded](media/service-admin-portal/manage-pbie-capacities-02.png)
...
* Значок (обязательно). Добавьте файл значка, который отображается в пользовательском интерфейсе Power BI Desktop.
...
Выберите "Применить", чтобы инициировать запрос на передачу. При успешном выполнении вы увидите в списке новый элемент. В случае сбоя может поступить соответствующее сообщение об ошибке.
