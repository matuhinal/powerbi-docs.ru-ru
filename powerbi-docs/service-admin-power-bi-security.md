...
Исключение составляют подключения к **службам SQL Server Analysis** через **локальный шлюз данных**; информационные панели сохраняются в кэше Power BI, однако для доступа к исходным отчетам или наборам данных требуется проверка подлинности соответствующего пользователя, причем доступ предоставляется только в том случае, если пользователь обладает достаточными правами для доступа к этим сведениям. Дополнительную информацию см. в разделе [Локальный шлюз данных: глубокое погружение](service-gateway-onprem-indepth.md).
