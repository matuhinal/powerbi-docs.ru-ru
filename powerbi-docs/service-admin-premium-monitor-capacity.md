...
Вкладка **Наборы данных** обеспечивает большую часть метрик приложения. Используйте четыре кнопки в верхней части вкладки, чтобы переходить к различным областям: **Сводка**, **Обновления**, **Запросы** и **Наборы данных**.

![вкладка "Наборы данных"](media/service-admin-premium-monitor-capacity/datasets-tab.png)

#### <a name="summary-area"></a>область "Сводка"

![кнопка "Сводка"](media/service-admin-premium-monitor-capacity/summary-button.png)
...
![кнопка "Обновления"](media/service-admin-premium-monitor-capacity/refreshes-button.png)
...
#### <a name="queries-area"></a>область "Запросы"

![кнопка "Запросы"](media/service-admin-premium-monitor-capacity/queries-button.png)

В области **Запросы** указано общее количество запусков запросов, общее количество подсчета ожиданий для активного запроса/прямого запроса, средняя/максимальная продолжительность, среднее/максимальное время ожидания (в миллисекундах), разделенных на наборы данных, рабочую область и группы за один час, за последние семь дней. На диаграммах ниже указано количество запросов, средняя продолжительность (в миллисекундах) и среднее время ожидания (в миллисекундах) по сравнению с потреблением памяти в ГБ, разделенные на группы за один час, которые сообщаются по местному времени. На двух верхних графиках справа указано пять лучших наборов данных по средней продолжительности запроса и времени ожидания, которое потребовалось для завершения запросов. Длительные запросы и длительное время ожидания указывают на горячий уровень хранилища емкости. Это может также означать, что один набор данных вызывает проблемы и требует дальнейшего изучения.

#### <a name="datasets-area"></a>область "Наборы данных"

![кнопка "Наборы данных"](media/service-admin-premium-monitor-capacity/datasets-button.png)
