...
| **Предельный объем памяти для запросов (%)** | Максимальный процент доступной памяти в рабочей нагрузке, который может использоваться для выполнения запроса многомерных выражений (MDX) или DAX. |
...
Отчеты с разбивкой на страницы позволяют запускать собственный код для отрисовки отчета. Например, можно динамически изменять цвет текста в зависимости от содержимого, для чего требуется дополнительная память. Power BI Premium выполняет отчеты с разбивкой на страницы в автономной области в пределах емкости. 
