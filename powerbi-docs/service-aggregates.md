...
* **Отклонение**.
* **Медиана**.  Показывает значение медианы (среднее). Это значение с тем же или меньшим числом элементов.  Если медиан две, Power BI вычисляет их среднее значение.
...
Вопрос. Почему я не могу агрегировать текстовые поля для источников данных SSAS?

Ответ. С активными подключениями к SSAS MD невозможно выполнять агрегирование со стороны клиента. Сюда входят функции first, last, avg, min, max и sum.
