...
* Файлы данных и определения потоков данных могут использоваться разработчиками в ИТ-отдел для эффективного использования данных Azure и служб искусственного интеллекта (ИИ), как показано в [примеры GitHub из служб данных Azure](https://aka.ms/cdmadstutorial)
* Разработчики в вашей организации могут интегрировать данные потока данных в внутренних приложений и бизнес-решений с использованием ресурсов для разработчиков для потоков данных и Azure
...
* **Учетная запись хранилища Azure с включенной функцией Gen2 хранилища Озера данных** 

> [!TIP]
> Если у вас нет подписки Azure, перед началом работы [создайте бесплатную учетную запись](https://azure.microsoft.com/free/).

> [!WARNING]
> Место хранения потока данных нельзя изменить после настройки. См. в разделе [рекомендации и ограничения](#considerations-and-limitations) в конце этой статьи для других важных элементов, которые следует учитывать.
...
> [!NOTE]
> Разрешить по крайней мере 30 минут для разрешения для объекта к Power BI на портале. При изменении разрешения на портале, разрешить эти разрешения могут отражаться в Power BI в течение 30 минут. 
...
5. Выделите и скопируйте как идентификаторы объектов для службы Power BI и Power Query в Интернете в результатах поиска. Эти значения понадобятся для вставки на следующих шагах.
...
После настройки учетной записи Gen2 хранилища Озера данных Azure на портале Azure, подключении к Power BI в **портал администрирования Power BI**. Вы также управлять хранения потока данных Power BI в **хранения потока данных** «параметры» на портале администрирования Power BI. Инструкции по запуску и базовому использованию см. в статье [Портал администрирования Power BI](service-admin-portal.md).

Подключить **учетную запись Azure Data Lake Storage 2-го поколения** можно следующим образом:

1. Перейдите к **параметры потока данных** вкладке **портал администрирования Power BI**

    ![Портал администрирования Power BI](media/service-dataflows-connect-azure-data-lake-storage-gen2/dataflows-connect-08b.png) 

2. Выберите **подключения вашей Gen2 хранилища Озера данных Azure** кнопки. Появляется следующее окно.
...
Чтобы предоставить разрешения на назначение хранилища, перейдите в раздел **параметры потока данных** вкладке **портал администрирования Power BI**. Для переключателя *Allow workspace admins to assign workspaces to this storage account* (Разрешение администраторам рабочих областей назначать области этой учетной записи хранения) необходимо установить значение **Разрешить**. Включив этот ползунок, нажмите кнопку **Применить**, чтобы изменения вступили в силу. 
...
3. Бен получит сообщение об ошибке, потому что он не был авторизован для папки CDM потока данных в озере.
