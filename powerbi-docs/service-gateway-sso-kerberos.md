...
### <a name="install-sap-bw-components"></a>Установите компоненты SAP BW

Если вы еще не настроили SAP gsskrb5 и gx64krb5 на клиентских компьютерах и сервере приложений SAP BW, выполните шаги в этом разделе. Если вы уже выполнили установку (создали учетную запись пользователя службы для сервера BW и сопоставили с ним имя субъекта-службы), можно пропустить некоторые части этого раздела.
...
### <a name="complete-the-gateway-configuration-for-sap-bw"></a>Проведите настройку шлюза для SAP BW
...
1. На контроллере домена Active Directory создайте пользователя службы (изначально это обычный пользователь Active Directory) для сервера приложений BW в среде Active Directory. Назначьте ему имя SPN.

    Назначенное SPN **должно** начинаться с SAP/. Что следует за SAP/, выбираете вы; один из вариантов — имя пользователя службы сервера BW. Например, если вы создаете BWServiceUser@\<DOMAIN\> в качестве пользователя службы, можно использовать имя субъекта-службы SAP/BWServiceUser. Один из способов установить сопоставление имени субъекта-службы — команда setspn. Например, чтобы задать имя субъекта-службы для пользователя службы, которого мы только что создали, следовало бы выполнить следующую команду из окна командной строки на компьютере контроллера домена: `setspn -s SAP/ BWServiceUser DOMAIN\ BWServiceUser`.

1. Предоставьте пользователю службы доступ к экземпляру сервера приложений BW:
...
1. Войдите на сервер через графический пользовательский интерфейс SAP GUI / Logon и задайте следующие параметры профиля с помощью транзакции RZ10:
    ...
    1. Задайте значение параметра профиля snc/identity/as как p:\<имя пользователя службы BW, только что созданного вами\>, например p:BWServiceUser@MYDOMAIN.COM. Обратите внимание на символы "p:", которые предшествуют имени субъекта-пользователя службы.
...
### <a name="map-azure-ad-users-to-sap-bw-users"></a>Сопоставьте пользователей Azure AD и пользователей SAP BW
...
### <a name="verify-sign-in-using-sso"></a>Проверьте единый вход в систему
...
### <a name="add-registry-entries"></a>Добавление записей реестра

Добавьте необходимые записи в реестр компьютера, на котором установлен шлюз. Затем укажите требуемые параметры конфигурации шлюза.
...
### <a name="set-azure-ad-properties"></a>Задайте свойства Azure AD

Укажите в свойстве msDS-cloudExtensionAttribute1 пользователя Active Directory, которой был сопоставлен пользователю BW на шаге сопоставления пользователей Azure AD и SAP BW, пользователя службы Power BI, для которого вы хотите включить единый вход Kerberos. Один из способов установить свойство msDS-cloudExtensionAttribute1 — с помощью оснастки Active Directory "Пользователи и компьютеры" (можно также использовать другие методы).
...
1. Выберите вкладку **Редактор атрибутов**. Если вы не видите ее, найдите инструкции о том, как ее включить, или используйте другой метод, чтобы задать свойство msDS-cloudExtensionAttribute1. Выберите один из атрибутов и затем используйте клавишу M для перехода к свойствам Active Directory, которые начинаются с этой буквы. Найдите свойство msDS-cloudExtensionAttribute1 и дважды щелкните его. Задайте в нем имя пользователя, которое вы используете для входа в службу Power BI. Нажмите кнопку **ОК**.
...
1. В поле **Имя партнера SNC** введите значение, хранящееся в параметре профиля сервера snc/identity/as, *добавив SAP/ между p: и остальной частью удостоверения*. Например, если удостоверение snc сервера — p:BWServiceUser@MYDOMAIN.COM, следует ввести p:SAP/BWServiceUser@MYDOMAIN.COM в поле ввода "Имя партнера SNC".
...
1. Установите флажок **Использовать единый вход через Kerberos для запросов DirectQuery** и нажмите кнопку **Применить**. Если проверка подключения не была успешной, убедитесь, что предыдущие шаги установки и настройки были выполнены правильно.

### <a name="test-your-setup"></a>Проверьте установку

Опубликуйте для проверки отчет DirectQuery из Power BI Desktop в службе Power BI. Убедитесь, что вы вошли в службу Power BI от имени пользователя, указанного в свойстве msDS-cloudExtensionAttribute1. Если установка выполнена успешно, вы сможете создать отчет на основе опубликованного набора данных в службе Power BI и извлекать данные с помощью визуальных элементов в отчете.
