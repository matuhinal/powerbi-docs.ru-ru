...
Power BI Desktop может подключаться к набору данных Power BI Premium, как если бы это была база данных модели, развернутая в Azure Analysis Services или SQL Server Analysis Services. В этом случае Power BI Desktop использует конечную точку XMLA. Тем не менее пользователям Power BI Desktop рекомендуется использовать вместо этого функцию активного подключения, созданную специально для наборов данных Power BI. Использование активного подключения обеспечивает улучшенные возможности обнаружения благодаря отображению уровня подтверждения для наборов данных. При этом пользователям не нужно отслеживать URL-адреса рабочих областей, так как они могут просто ввести имя набора данных. Дополнительные сведения см. в статье [Подключение к наборам данных в службе Power BI из приложения Power BI Desktop](desktop-report-lifecycle-datasets.md).
