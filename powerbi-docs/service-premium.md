...
| Узел емкости | Число виртуальных ядер<br/>*(Серверная часть и интерфейс)*  | Виртуальные ядра сервера <sup>[1](#fn1)</sup> | Виртуальные ядра интерфейса <sup>[2](#fn2)</sup> | Ограничения для подключений DirectQuery и активных подключений | Максимальное количество одновременных обновлений |  Доступность
| --- | --- | --- | --- | --- | --- | --- | --- |
| EM1 (ежемесячная подписка) |1 виртуальное ядро |0,5 виртуальных ядер, 2,5 ГБ ОЗУ |0,5 виртуальных ядра |3,75 в секунду |  1 | Доступно |
| EM2 (ежемесячная подписка) |2 виртуальных ядра |1 виртуальное ядро, 5 ГБ ОЗУ |1 виртуальное ядро |7,5 в секунду |  2 | Доступно |
| EM3 (ежемесячная подписка) |4 виртуальных ядра |2 виртуальных ядра, 10 ГБ ОЗУ |2 виртуальных ядра | | 3 |  Доступно |
| P1 |8 виртуальных ядер |4 виртуальных ядра, 25 ГБ ОЗУ |4 виртуальных ядра |30 в секунду | 6 | Доступно (вариант ежемесячной подписки также доступен) |
| P2 |16 виртуальных ядер |8 виртуальных ядер, 50 ГБ ОЗУ |8 виртуальных ядер |60 в секунду | 12 | Доступно |
| P3 |32 виртуальных ядра |16 виртуальных ядер, 100 ГБ ОЗУ |16 виртуальных ядер |120 в секунду | 24 | Доступно |
| | | | | | | |
...
При использовании отчетов с разбивкой на страницы учитывайте, что можно запускать собственный код для отрисовки отчета (например, динамически изменять цвет текста в зависимости от содержимого). С учетом этого емкость Power BI Premium резервируется путем выполнения отчетов с разбивкой на страницы в автономной области в пределах емкости. Мы выделяем для этой области максимальный объем памяти, который вы настроили, независимо от активности рабочей нагрузки. Если в этой же емкости вы используете отчеты или потоки данных Power BI, выделяйте для отчетов с разбивкой на страницы не слишком много памяти, чтобы они не ухудшали работу других рабочих нагрузок.
