...
Теперь получать актуальные версии важных панелей мониторинга и отчетов стало проще, чем когда-либо. Подпишитесь на страницы наиболее важных отчетов и панели мониторинга, и Power BI будет отправлять вам моментальные снимки по электронной почте. Вы можете указать, с какой частотой это нужно делать: ежедневно или еженедельно. В сообщении электронной почты и снимке отчета будет использоваться язык, заданный в настройках Power BI. Если язык не задан, Power BI использует язык, указанный в браузере.

Чтобы просмотреть или настроить параметры языка, выберите значок ![](media/service-report-subscribe/power-bi-settings-icon.png) **> Параметры > Общие > Язык**.
