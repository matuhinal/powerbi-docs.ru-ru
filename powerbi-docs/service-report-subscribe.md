---
title: Подписка себя и других на отчеты и панели мониторинга — Power BI
description: Узнайте, как подписаться на моментальный снимок страницу отчета Power BI, панель мониторинга или отчета с разбивкой на страницы себя и других.
author: maggiesMSFT
manager: kfile
ms.reviewer: ''
featuredvideoid: ''
...
Вы можете подписаться на себя и вашим коллегам страницы отчетов, панелей мониторинга и отчеты с разбивкой на страницы, наиболее важных для вас. Power BI отправляет по электронной почте моментальный снимок в ваш почтовый ящик. Вы можете указать, с какой частотой это нужно делать: ежедневно, еженедельно или после первоначального обновления данных (раз в день).  Если вы ежедневно или еженедельно, вы можете выбрать время вы бы хотели иметь запуска подписки.  В целом можно настроить до 24 разных рассылок в день для каждой страницы отчета и панели мониторинга.
...
## <a name="subscribe-to-a-dashboard-report-page-or-paginated-report"></a>Подписаться на панель мониторинга, страницы отчета или приступить к созданию отчета

Похожа ли при подписке на панели мониторинга, отчетов, или с разбиением на страницы отчетов, процесс. Одна кнопка позволяет оформить подписки на панели мониторинга и отчеты службы Power BI.

Подписка на отчеты с разбивкой на страницы будет немного отличаться. См. в разделе [подписаться на отчет с разбивкой на страницы в службе Power BI себя и других](paginated-reports-subscriptions.md) подробные сведения.
...
7. Щелкните **Сохранить и закрыть**. Подписчики получают сообщение электронной почты и моментальные снимки страницы отчета или панели мониторинга с выбранной частотой и в указанное вами время. В общем можно создать до 24 подписок на отчет или панель мониторинга и указать уникальных получателей, значения времени и частоты для каждой подписки.  Все подписки, для которых для отчета или панели мониторинга задана частота **После обновления данных**, будут отправлять сообщение только после первого запланированного обновления.   
   ...
   > [!TIP]
   > Хотите отправлять электронное письмо из подписки немедленно или по требованию в любое время? Выберите **Выполнить** для подписок в панели мониторинга или отчете, которые будут отправлять сообщение. Появится уведомление, информирующее об отправке электронного письма всем пользователям конкретной подписки.  Вы можете это так часто, как это необходимо. Она не учитывается как один из 24 максимально допустимых запланированных запусков подписки в день на каждый отчет или панель мониторинга. Это не вызывает обновления данных из базового набора данных. 
   > 
   > 
...
* Панели мониторинга с более чем 25 закрепленными плитками или 4 закрепленными страницами динамического отчета могут отображаться не полностью в рассылках, отправляемых пользователям.  Подписки на информационных панелях, через эти количества элементов мозаики не блокируются. Тем не менее считаются не поддерживается при возникновении проблем. Попробуйте изменить их соответствующим образом могут быть удалены в поддерживаемом диапазоне.
* При настройке подписок электронной почты, имейте в виду, что происходит задержка между при запуске задания подписки и точное время отправки сообщения электронной почты.  Чтобы свести к минимуму задержки между ними, настройте другое время в расписание обновления данных, чем при вашей подписки по электронной почте по расписанию.
* Для мониторинга подписок электронной почты Если все плитки на уровне строк (RLS) применяется, не отображать эти плитки.  
* Для подписок на отчеты по электронной почте Если этот набор данных использует безопасность на уровне СТРОК, можно создать подписку для себя. Невозможно подписать других пользователей отчета обеспечения безопасности на уровне строк (RLS).
...
- [Подписаться на отчет с разбивкой на страницы в службе Power BI себя и других](paginated-reports-subscriptions.md)
