...
Если вы хотите добавить фильтр URL-адреса в отчет в приложении, форматирование будет немного отличаться. Ссылки на отчеты в приложении имеют параметр запроса (ctid), который добавляется к URL-адресу. Разделяйте параметры запроса амперсанд (&). Сохранить «? filter =» и переместить в конец URL-адрес, параметр ctid знаком амперсанда (&). 

как в следующем примере:

App.powerbi.com/Groups/Me/Apps/*идентификатор приложения*/reports/*идентификатор отчета*/ReportSection? filter =*таблицы*/*поле* eq "*значение*&" ctid =*ctid*
...
![Отчет, отфильтрованный по Северная Каролина](media/service-url-filters/power-bi-report4.png)
...
Для типов данных **Date** и **DateTimeOffset** Power BI поддерживает OData V3 и V4.  Даты представляются в формате модели EDM (2019-02-12T00:00:00), поэтому при указании даты в виде «Гггг-мм-дд», Power BI интерпретирует его как "гггг-мм-DDT00:00:00".

Почему это различие имеет значение? Предположим, создается параметр строки запроса **gt таблицы/Дата "2018-08-03"** .  Будет ли дата 3 августа 2018 г. включена в результаты или же результаты будут начинаться с даты 4 августа 2018 г.? Так как Power BI преобразует запрос так, чтобы **gt таблицы/Дата "2018-08-03T00:00:00"** , результаты содержат любые даты, имеющих части времени ненулевое значение, так как эти даты будет больше **"2018-08-03T00:00:00"** .
...
|**[Столбец]**     |  [ — 0x0058, ] — 0x0050       |  _x0058_Column_x0050_       |
...
* Фильтрация строки запроса не работает с [опубликовать в Интернете](service-publish-to-web.md) или [Экспорт в PDF](consumer/end-user-pdf.md).
