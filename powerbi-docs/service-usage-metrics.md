...
description: "Документация по просмотру метрик использования для панелей мониторинга и отчетов Power BI. Оцените и повысьте эффективность с помощью метрик использования для создателей содержимого."
services: powerbi
documentationcenter: 
author: mihart
manager: kfile
...
> [!NOTE]
> Метрики использования собирают сведения об использовании при внедрении панелей мониторинга и отчетов, но не при [публикации в Интернете](service-publish-to-web.md).
> 
> 
...
> **Примечание.** Функция метрик использования записывает сведения об использовании для всех пользователей бесплатной лицензии и версии Pro. Но для запуска и доступа к данным метрик использования требуется лицензия Pro.
> 
> 

## <a name="about-the-usage-metrics-report"></a>Отчет на основе метрик использования
Метрики использования предоставляются на панелях мониторинга и отчетах в выбранной рабочей области. Чтобы получить доступ к данным метрик использования для конкретной панели мониторинга или отчета, требуется:    
•   доступ на изменение данных к панели мониторинга или отчету;   
•   лицензия Pro.
...
4. Чтобы открыть результаты, выберите **Просмотр метрик использования**.
    ...
    Метрики использования будут очень полезны при развертывании и поддержке панелей мониторинга и отчетов Power BI. Интересно, какие страницы отчета наиболее полезны и какие из них следует исключить? Выполните срез по страницам отчета, чтобы узнать. Интересно, необходимо ли создавать макет панели мониторинга для мобильного устройства? Отчет на основе метрик использования проинформирует вас, сколько пользователей получают доступ к содержимому через мобильные приложения и веб-браузер.
5. При необходимости закрепите визуализации на панели мониторинга, чтобы легче их отслеживать, а также предоставлять к ним доступ другим пользователям.
...
Используйте действие **Сохранить как**, чтобы преобразовать отчет на основе метрик использования в полнофункциональный отчет Power BI, который можно настраивать и к которому можно предоставлять доступ. После создания персонализированной копии вы сможете получить полный доступ к базовому набору данных, что позволяет полностью настроить отчет на основе метрик использования в соответствии с вашими потребностями. Вы также можете использовать Power BI Desktop для создания настраиваемых отчетов на основе метрик использования с помощью [динамического подключения к компоненту службы Power BI](https://powerbi.microsoft.com/blog/connecting-to-datasets-in-the-power-bi-service-from-desktop).
...
При создании копии с помощью команды "Сохранить как" (персонализировать) Power BI преобразовывает готовый отчет только для чтения в полнофункциональный отчет.  На первый взгляд он выглядит точно так же. Тем не менее теперь можно открыть отчет в режиме правки, добавить новые визуализации, фильтры и страницы, изменить или удалить имеющиеся визуализации и многое другое. Power BI также создает набор данных, содержащий метрики использования для всех панелей мониторинга или отчетов в рабочей области.

> **Совет.** Чтобы разрешить Power BI доступ к метрикам использования для всех панелей мониторинга или отчетов в рабочей области, [удалите фильтр на уровне страницы](#remove-the-filter-to-see-all-the-usage-metrics-data-in-the-workspace).
> 
> 

1. В готовом отчете на основе метрик использования выберите **Файл > Сохранить как**. Служба Power BI конвертирует отчет на основе метрик использования в полнофункциональный отчет Power BI. Это называется *персонализированным* отчетом на основе метрик использования.
...
3. Кроме того, можно открыть новый набор данных и создать отчет с нуля.
   
    ![](media/service-usage-metrics/power-bi-new-dataset.png)
4. Новый отчет сохраняется в текущей рабочей области, а также добавляется в список содержимого **Последние**.
   
    ![](media/service-usage-metrics/power-bi-new-report.png)

### <a name="remove-the-filter-to-see-all-the-usage-metrics-data-in-the-workspace"></a>Удаление фильтра для просмотра всех данных метрик использования в рабочей области
