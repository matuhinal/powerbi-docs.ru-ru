...
1. Создайте новый график для отслеживания **валовой прибыли за последний год %** по **FiscalMonth**. Нажмите кнопку с многоточием, чтобы выполнить сортировку по **месяцам** и по **возрастанию**.  
...
3. Для **ось y (столбец)** , задайте **позиции** для **слева**, задайте **Title** для **на**,  **Стиль** для **Показать только заголовок**, и **отображаемые единицы** как **миллионы**.

   ![](media/power-bi-visualization-combo-chart/power-bi-open-y.png)
4. В разделе **ось y (столбец)** , прокрутите вниз, пока не увидите **Показать вспомогательную**. Так как существует множество возможностей для оси Y, может потребоваться использовать обе полосы прокрутки. Дополнительный раздел Show отображаются параметры форматирования диаграммы строки часть комбинированную диаграмму.
