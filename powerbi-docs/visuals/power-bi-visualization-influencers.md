...
Анализируемая метрика должна быть категориальным или числовым полем (статистические выражения и меры пока не поддерживаются).
...
7. **Линия среднего значения**: вычисляется среднее значение для всех других возможных значений **темы**, за исключением **удобства использования**. Поэтому вычисление применяется ко всем значениям, выделенным черным цветом. Это указывает, какой процент других **тем** дает нам низкую оценку. Другими словами, получая оценку от пользователя, мы также получаем описание причины или тему для оценки. Некоторые темы: удобство использования, скорость и безопасность. 
   ...
   **Тема — удобство использования** является вторым по значимости ключевым фактором влияния на низкую оценку в соответствии с визуальным элементом в области слева. Если взять среднее значение всех других тем и их вклада в **низкой** оценке, получается результат, выделенный красным цветом. Среди всех других тем только 11,35 % превышают **удобство использования**.
...
Просмотрите это видео, чтобы научиться создавать визуализацию для ключевых факторов влияния. После этого выполните указанные ниже действия, чтобы создать ее. 
...
2. Переместите метрику, которую необходимо изучить, в поле **Анализ**. Поле **Анализ** поддерживает только категориальные (или дискретные) переменные. Чтобы узнать, почему пользователи дают службе низкие оценки, выберите пункты **Таблица пользователей** > **Оценка**. 
3. Переместите поля, которые, как вы считаете, могут повлиять на **Оценку**, в поле **Объяснение по**. Можно переместить любое количество полей. В данном случае начните со следующих:
    ...
    - Размер компании 
    - Тема 
1. Чтобы сконцентрироваться на отрицательных оценках, выберите значение **Низкое** в раскрывающемся списке **Что вызывает у "Оценка" значение**.  
...
В предыдущем примере все пояснительные факторы имеют связь "один к одному" или "многие к одному" с метрикой. В этом случае каждый показатель связан ровно с одной темой. Это была основная тема отзыва пользователя. Аналогичным образом пользователи находятся в одной стране, имеют один тип членства и одну роль в своей организации. Пояснительные факторы уже являются атрибутами пользователя, а преобразования не требуются. Визуальный элемент можно сразу использовать без них. 
...
В организации из данного примера есть три роли: потребитель, администратор и издатель. Потребитель является ведущим фактором, оказывающим влияние на низкую оценку. 
...
При перемещении числового поля в поле **Анализ** вы можете выбрать способ обработки этого сценария. Поведение визуального элемента можно изменить, перейдя в **область форматирования** и переключившись между типами анализа **Категориальный** и **Непрерывный**.
...
Визуализация ключевых факторов влияния сейчас находится в общедоступной предварительной версии и имеет несколько ограничений. Функциональные возможности, недоступные в настоящее время, включают в себя: 
...
В этом примере данные были сведены для создания столбцов для браузеров, мобильных устройств и планшетов. Теперь их можно использовать в поле **Объяснение по**. Все устройства оказываются факторами влияния, и браузер оказывает наибольшее влияние на оценку пользователя.
