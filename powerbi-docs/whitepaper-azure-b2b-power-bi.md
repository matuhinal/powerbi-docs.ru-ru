...
2. Приглашение гостевых пользователей
    ...
    **Запланированные приглашения**
    ...
    После того, как гостевой пользователь перейдет по ссылке-приглашению, в клиенте Contoso Azure AD он (она) сможет получить доступ к содержимому.
    ...
    **Несистематические приглашения**
    ...
    Аналитик при публикации может добавить сторонних пользователей в список доступа приложения. Гостевой пользователь получает приглашение и после его принятия, он автоматически перенаправляется к содержимому Power BI.
...
### <a name="approach-2-contoso-assigns-power-bi-pro-licenses-to-guest-users"></a>Подход 2. Contoso назначает гостевым пользователям лицензии Power BI Pro
...
Теперь, если пользователь открывает приложение, доступ к которому ему были предоставлен, перед ним будет отображатся только отчет с данными по Европе.
...
Теперь, когда гостевой пользователь открывает отчет, он видит только продажи из группы А:
