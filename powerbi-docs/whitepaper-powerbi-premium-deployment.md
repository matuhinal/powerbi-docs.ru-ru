...
ms.locfileid: "73432372"
---
# <a name="deploying-and-managing-power-bi-premium-capacities"></a>Развертывание емкостей Power BI Premium и управление ими

**Сводка:** Power BI Premium обеспечивает более согласованную производительность, поддержку больших объемов данных и гибкость унифицированной самостоятельной и корпоративной платформы бизнес-аналитики для всех пользователей в Организации. Этот технический документ уровня 300 был написан специально для администраторов Power BI, а также для авторов и издателей содержимого. Он содержит сведения о возможностях Power BI Premium, а также о проектировании, развертывании, отслеживании и устранении неполадок масштабируемых решений.

**Автор:** [Питер Майерс](https://www.linkedin.com/in/peterjsmyers) (MVP по платформе данных и независимый эксперт по бизнес-аналитике с побитовыми решениями)

**Технические рецензенты:** Адам Сакстон, Акшаи Мирчандани, Бхавикный магазин, Дэвид Магар, Джош Каплан, Майкл Блайт, Нимрод Шалит, Оливиер Матрат, Свати Гупта

**Применимо к:** Служба Power BI, Power BI Premium и емкости Power BI Embedded Azure
...
Она предоставляется двумя способами: в качестве облачной службы и в качестве локального решения для создания отчетов с именем **сервер отчетов Power BI**. \[[1](#endnote-01)\]
...
Рабочие области Power BI находятся в пределах емкости и представляют собой контейнеры безопасности, совместной работы и развертывания. У каждого пользователя Power BI есть личная рабочая область, так называемая **Моя рабочая область**. Можно создать дополнительные рабочие области, чтобы обеспечить совместную работу и развертывание, и они называются **рабочими областями**. По умолчанию рабочие области, включая личные рабочие области, создаются в общей емкости.
...
Хранение всего содержимого Power BI и управление им осуществляется в рабочих областях, которые являются для него контейнерами. Каждый пользователь Power BI имеет собственную персональную рабочую область, но рекомендуется создать рабочие области. Рабочие области обеспечивают совместное владение содержимым и возможность совместной работы над содержимым. Они также предоставляют возможность размещать и распространять содержимое среди широкой аудитории в качестве приложений.
...
Потоки данных можно создавать и управлять только в рабочих областях, и они хранятся в виде сущностей в модели Common Data Model (CDM) в Azure Data Lake Storage 2-го поколения. Как правило, для них запланировано регулярное обновление, обеспечивающее хранение актуальных данных.
...
Существует два типа отчетов: Power BI отчеты и отчеты с разбивкой на страницы.
...
## <a name="introducing-power-bi-premium"></a>Введение в Power BI Premium
...
Альтернативный подход заключается в приобретении подписки Azure Power BI Embedded с одним семейством **SKU: SKU** (a1-A6) для внедрения и тестирования емкости.
...
Как описано в разделе подписки и лицензирование, существует два семейства номеров SKU Power BI Premium: EM и P. Все Power BI Premium SKU доступны в виде узлов емкости, каждый из которых представляет собой набор ресурсов, состоящий из процессора, памяти и хранилища. Помимо ресурсов каждый номер SKU содержит операционные ограничения на количество подключений DirectQuery (DQ) и активных подключений в секунду и количество параллельных обновлений моделей.
...
Модели импорта должны быть полностью загружены в память, чтобы к ним можно было отправлять запросы или выполнять их обновление. Служба Power BI управляет использованием памяти с помощью сложных алгоритмов, обеспечивая максимально возможное использование доступной памяти, а также может повысить производительность. в то же время можно хранить много моделей импорта (до 100 ТБ на емкость Premium). Если объем дискового накопителя превышает поддерживаемую память (и для запроса и обновления требуется дополнительная память), они не могут быть загружены в память в то же время.
...
Рабочие области можно назначить емкости Premium на**портале** **администрирования Power BI**или для рабочей области — в области **Рабочая** область.
...
Мониторинг емкостей Premium предоставляет администраторам сведения о выполнении емкостей. Для мониторинга емкости можно использовать [приложение метрики емкости Power BI Premium](service-admin-premium-monitor-capacity.md) или [портал администрирования Power BI](service-admin-premium-monitor-portal.md).
...
- **Обновления выполняются быстрее** : только подмножество таблицы нуждается в загрузке, уменьшая использование ЦП и памяти, а параллелизм может быть выше при обновлении нескольких секций.
- **Обновления происходят только при необходимости** : политики добавочного обновления могут быть настроены для загрузки только при изменении данных.
- **Обновления более надежны** : более короткие подключения к временным системам источников данных менее уязвимы для отключения.
- **Модели остаются обрезанными** : политики добавочного обновления могут быть настроены на автоматическое удаление журнала за пределами скользящего окна времени.
...
- **Размер модели и характеристики данных** . модели импорта должны быть полностью загружены в память, чтобы разрешить выполнение запросов или обновление. Наборам данных активных подключений и DQ может потребоваться значительная загруженность процессора, а также, возможно, значительный объем памяти для оценки сложных мер или правил RLS. Объем памяти и размер процессора, а также пропускная способность запросов активных подключений и DQ ограничены размером емкости.
- **Параллельные активные модели** . Параллельное выполнение запросов различных моделей импорта обеспечит высокую скорость реагирования и производительность, когда они остаются в памяти. Для размещения всех моделей с большим количеством запросов необходим достаточный объем памяти. Кроме того, для их обновления потребуется дополнительная память.
- **Импорт модели обновление** : тип обновления (полная или добавочная), длительность и сложность Power Query запросов и логики вычисляемых таблиц и столбцов могут повлиять на память и особенно на загрузку процессора. Одновременные обновления ограничены размером емкости (в 1,5 раза больше количества серверных виртуальных ядер, округленного в большую сторону).
- **Параллельные запросы** . Многие параллельные запросы могут привести к неотвечающим отчетам при превышении предельной емкости для подключений к процессору или по аккредитиву или DQ. В первую очередь это касается страниц отчетов, которые содержат большое количество визуальных элементов.
- **Потоки данных, отчеты с разбивкой на страницы и функции искусственного интеллекта** . емкость можно настроить для поддержки потоков данных, отчетов с разбивкой на страницы и функций AI, каждый из которых требует настраиваемого максимального объема памяти в процентах. Память выделяется динамически для потоков данных, но статически для отчетов с разбивкой на страницы и рабочей нагрузки ИИ.
...
Важно, чтобы тесты были повторяемыми: тесты должны выполняться несколько раз, и они должны доставлять примерно одинаковый результат каждый раз. Среднее значение по этим результатам можно использовать для экстраполяции и оценки рабочей нагрузки в реальных рабочих условиях.

Если у вас уже есть емкость и отчеты, для которых требуется выполнить нагрузочный тест, используйте [средство формирования нагрузки PowerShell](https://aka.ms/PowerBILoadTestingTool) для быстрого создания нагрузочного теста. Это средство позволяет оценить, сколько экземпляров каждого отчета ваша емкость может выполнять в течение часа. С помощью него можно оценить способность емкости к отрисовке отдельного отчета или параллельной отрисовке нескольких разных отчетов. Дополнительные сведения см. в видеоролике [Microsoft Power BI: емкость Premium](https://www.youtube.com/watch?time_continue=1860&v=C6vk6wk9dcw).

Чтобы создать более сложный тест, разработайте приложение нагрузочного тестирования для имитации реалистичной рабочей нагрузки. Дополнительные сведения см. в вебинаре [Load Testing Power BI Applications with Visual Studio Load Test](https://www.youtube.com/watch?v=UFbCh5TaR4w) (Нагрузочное тестирование приложений Power BI с использованием нагрузочного теста Visual Studio).
