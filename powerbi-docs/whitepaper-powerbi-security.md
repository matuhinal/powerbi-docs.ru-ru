...
На следующем рисунке показано, как службы Power BI, предоставляемые в удаленном регионе в рамках среды с поддержкой нескольких регионов, маршрутизируются через **внутренний кластер Power BI**, где осуществляется подключение к удаленной виртуальной машине подписки Power BI клиента.
...
Сейчас служба Power BI доступна в определенных регионах, обслуживаемых центрами обработки данных, согласно описанию в [центре управления безопасностью Майкрософт]((https://www.microsoft.com/TrustCenter/CloudServices/business-application-platform/data-location). По следующей ссылке приведена карта центров обработки данных Power BI, где вы можете навести указатель на регион для просмотра расположенных там центров обработки данных:
...
|  |Импорт  |DirectQuery  |Live Connect  |
|---------|---------|---------|---------|
|Схема     |     X    |    X     |         |
|Данные строк     |    X     |         |         |
...
1. Data
    ...
    1\. Локальная версия Analysis Services и DirectQuery — в службе Power BI не хранится ничего.

    2\. Извлечение, преобразование и загрузка — данные зашифрованы в хранилище BLOB-объектов Azure, однако все данные, находящиеся в хранилище BLOB-объектов Azure службы Power BI, используют [Шифрование службы хранилища Azure (SSE)](https://docs.microsoft.com/azure/storage/common/storage-service-encryption), также называемое шифрованием на стороне сервера. Multi-Geo также использует SSE.

    В. Отправка данных версии 1 — данные зашифрованы в хранилище BLOB-объектов Azure, однако все данные, находящиеся в хранилище BLOB-объектов Azure службы Power BI, используют [Шифрование службы хранилища Azure (SSE)](https://docs.microsoft.com/azure/storage/common/storage-service-encryption), также называемое шифрованием на стороне сервера. Multi-Geo также использует SSE. Push-уведомлений данных версии 1 были прекращена 2016. 
...
1. Метаданные (определение отчета)
   ...
   1\. Отчеты могут иметь формат Excel для Office 365 или Power BI. В зависимости от типа отчета для метаданных применимо следующее:
        
    &ensp; &ensp; . Отчет Excel метаданных хранится в зашифрованном виде в SQL Azure. Метаданные также хранятся в Office 365.

    &ensp; &ensp; б. Отчеты Power BI хранятся в зашифрованном виде в базе данных Azure SQL.
...
3. Кэши
    ...
    &ensp; &ensp; б. Для отчетов Power BI данные для отображаемых визуальных элементов кэшируются в зашифрованном виде в базе данных SQL Azure.
...
Долгосрочное устройства, устройства, памяти, который сохраняется без константы power. Ниже описаны данные, которые временно хранятся на энергонезависимых устройствах. 
...
3. Учетные данные для исходных источников данных
    ...
    1\. Локальная версия Analysis Services — ничего не хранится
    ...
    > [!NOTE]
    > Функция создания модели со стороны службы не поддерживаются, начиная с 2017 г.
...
Каждый источник данных пользователя устанавливает соединения на основании своего учетного имени и обращается к данным с этими учетными данными. После этого пользователи могут создавать запросы, панели мониторинга и отчеты на основе базовых данных.
...
* Подробный ответ на этот вопрос приведен по следующей ссылке: [Портам шлюза](/data-integration/gateway/service-gateway-communication#ports)
...
* При установке и настройке шлюза администратор вводит **ключ восстановления** шлюза. Что **ключ восстановления** используется для формирования со строгим **AES** симметричный ключ. **RSA** в то же время также создается асимметричный ключ.
    ...
    Когда пользователь вводит учетные данные источника данных в пользовательском интерфейсе службы Power BI, они шифруются с помощью открытого ключа в браузере. Шлюз расшифровывает учетные данные, используя закрытый ключ RSA и повторно шифрует их с помощью симметричного ключа AES перед сохранением данных в службе Power BI. Таким образом, у службы Power BI никогда нет доступа к незашифрованным данным.
...
* Шлюз поддерживает два следующих протокола связи:
  ...
  - **HTTPS – WebSockets через HTTPS + TLS**: этот протокол использует только порт 443. WebSocket инициируется одним сообщением HTTP CONNECT. После установления канала связь осуществляется, главным образом, посредством TCP+TLS. Можно принудительно подключить шлюз для использования этого протокола, изменив параметр, описанный в [статье о шлюзах локальной](/data-integration/gateway/service-gateway-communication#force-https-communication-with-azure-service-bus).
...
**Для шаблонов приложений Майкрософт выполняет любой безопасности или конфиденциальности оценки шаблона приложения перед публикацией элементов в коллекции?**
* Нет. Издатель приложения отвечает за содержимое при ответственности клиента будет просмотреть и определить, следует ли доверять издателю приложения шаблона. 

**Существуют приложения шаблона, которые можно отправить сведения за пределы сети клиента?**
* Да. Отвечает за клиента см. в политике конфиденциальности издателя и определите необходимость установки шаблона приложения в клиенте. Кроме того издатель отвечает требуется уведомлять о поведение и возможности приложения.
