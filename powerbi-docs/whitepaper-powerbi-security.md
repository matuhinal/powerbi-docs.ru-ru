...
ms.locfileid: "56893361"
---
# <a name="power-bi-security-whitepaper"></a>Технический документ по обеспечению безопасности в Power BI

**Сводка:** Power BI — это предложение веб-службы программного обеспечения (*SaaS* или программное обеспечение как услуга) от корпорации Майкрософт, позволяющее легко и быстро создавать панели мониторинга, отчеты, наборы данных и визуализации самостоятельной бизнес-аналитики. С помощью Power BI можно подключиться к множеству разных источников данных, комбинировать и формировать данные из этих подключений, а также создавать отчеты и панели мониторинга, которые можно использовать совместно с другими пользователями.

**Автор:** Дэвид Исемингер

**Технические рецензенты:** Pedram Rezaei (Педрам Резае), Cristian Petculescu (Кристиан Петкулеску), Siva Harinath (Сива Харинт), Tod Manning (Тод Мэннинг), Haydn Richardson (Хайдн Ричардсон), Adam Wilson (Адам Уилсон), Ben Childs (Бен Чайлдс), Robert Bruckner (Роберт Брюкнер), Sergei Gundorov (Сергей Гундоров), Kasper de Jonge (Каспер де Йонге)

**Область применения:** Power BI SaaS, Power BI Desktop, Power BI Embedded, Power BI Premium
...
Работа службы Power BI регулируется [условиями использования Microsoft Online Services](http://www.microsoftvolumelicensing.com/DocumentSearch.aspx?Mode=3&amp;DocumentTypeId=31) и [корпоративным заявлением о конфиденциальности Майкрософт](http://www.microsoft.com/privacystatement/OnlineServices/Default.aspx). Расположение обработки данных описано в условиях использования Microsoft Online Services. Основным источником сведений о соответствии для Power BI является [центр управления безопасностью Майкрософт](https://www.microsoft.com/trustcenter). Группа Power BI усердно работает, чтобы предложить клиентам последние инновации и повышение производительности. Сейчас Power BI относится к уровню D [платформы соответствия Office 365](http://go.microsoft.com/fwlink/p/?LinkID=618494).
...
**Важно!** Крайне важно отметить, что через общедоступный Интернет доступны _только_ роли Управления API Azure (**APIM**) и шлюза (**GW**). Они обеспечивают проверку подлинности, авторизацию, защиту от DDoS, регулирование, балансировку нагрузки, маршрутизацию и другие функции.
...
### <a name="multiple-geographies-multi-geo---preview"></a>Поддержка нескольких регионов (Multi-Geo) — предварительная версия

Некоторым организациям, исходя из потребностей бизнеса, требуется, чтобы служба Power BI присутствовала в нескольких географических регионах. Например, организация, клиент Power BI которой находится в США, может осуществлять коммерческую деятельность и в других географических регионах, таких как Австралия, поэтому ей нужно, чтобы данные и службы Power BI оставались в этом удаленном регионе.  Начиная со второй половины 2018 г. организации, клиент которых находится в одном географическом регионе, могут также обращаться к ресурсам Power BI в другом географическом регионе при условии правильной подготовки. Для удобства в настоящем документе эта функция называется **поддержкой нескольких регионов**.

При работе в разных географических регионах следует учитывать определенные технические аспекты, которые разъяснены в этом документе. Важные замечания включают в себя следующее:
...
- Отчеты в файлах PBIX или XLSX в удаленном регионе, публикуемые в Power BI, иногда имеют копию или теневую копию в хранилище BLOB-объектов Azure Power BI. В такой ситуации эти данные шифруются с использованием Шифрования службы хранилища Azure (SSE).
- При перемещении данных из одного региона в другой в среде с поддержкой нескольких регионов сборка мусора в регионе, откуда были перемещены данные, выполняется в течение 7–10 дней, при этом копия данных, перемещенная из исходного региона, удаляется.

На следующем рисунке показано, как службы Power BI, предоставляемые в удаленном регионе в рамках среды с поддержкой нескольких регионов, маршрутизируются через **внутренний кластер Power BI**, где осуществляется подключение к удаленной виртуальной машины подписки Power BI клиента.

![Поддержка нескольких регионов](media/whitepaper-powerbi-security/powerbi-security-whitepaper_07.png)
...
- [Регионы Azure](http://azure.microsoft.com/regions/) — сведения о глобальном присутствии и расположениях Azure
- [Службы Azure по регионам](http://azure.microsoft.com/regions/#services) — полный перечень служб Azure (как инфраструктуры, так и платформы), предоставляемых корпорацией Майкрософт в каждом регионе.

Сейчас служба Power BI доступна в следующих регионах, обслуживаемых указанными ниже основными центрами обработки данных:
...
Кроме того, служба Power BI по-разному управляет данными в зависимости от того, осуществляется ли доступ к ним с помощью **DirectQuery** или _нет_. Поэтому существуют две категории пользовательских данных для Power BI: данные, к которому обращаются с помощью DirectQuery, и данные, к которым не обращаются с помощью DirectQuery.
...
Запрос, **не** использующий DirectQuery, состоит из коллекции запросов DAX, которые _не_ переводятся непосредственно на собственный язык базового источника данных. Запросы, отличные от DirectQuery, не включают учетные данные для базовых данных, и базовые данные загружаются в службу Power BI, если только это не локальные данные, обращение к которым осуществляется через [Power BI Gateway](https://powerbi.microsoft.com/documentation/powerbi-gateway-enterprise/), в случае чего запрос хранит только ссылки на локальные данные.
...
2. Статические данные
   ...
   Статические данные включают в себя артефакты, например фоновые изображения и пользовательские визуальные элементы.
    ...
    а. Для отчетов, созданных с помощью Excel для Office 365, ничего не сохраняется.

    б. Для отчетов Power BI статические данные сохраняются и шифруются в хранилище BLOB-объектов Azure.

3. Кэши а. Для отчетов, созданных с помощью Excel для Office 365, ничего не кэшируется.

    б. Для отчетов Power BI данные для отображаемых визуальных элементов кэшируются в зашифрованном виде в базе данных SQL Azure.
...
2. Статические данные, включающие в себя артефакты, например фоновые изображения и пользовательские визуальные элементы, хранящиеся зашифрованными в хранилище BLOB-объектов Azure.
...
Ниже описаны данные, которые временно хранятся на энергонезависимых устройствах.
...
Для каждого источника данных пользователь устанавливает соединения на основе своего имени для входа и обращается к данным с помощью этих учетных данных. После этого пользователи могут создавать запросы, панели мониторинга и отчеты на основе базовых данных.
...
ExpressRoute — это служба Azure, которая позволяет создавать частные подключения между центрами обработки данных Azure (где находится Power BI) и локальной инфраструктурой или создавать частные подключения между центрами обработки данных Azure и средой совместного размещения. Дополнительные сведения см. в статье [Power BI и ExpressRoute](https://powerbi.microsoft.com/documentation/powerbi-admin-power-bi-expressroute/).
...
Приложения Power BI Mobile не просматривают папки на устройстве. Вы можете ознакомиться с дополнительными сведениями об [автономных данных в приложениях Power BI Mobile](https://powerbi.microsoft.com/documentation/powerbi-mobile-offline-android/).

Все три платформы, для которых доступна служба Power BI Mobile, поддерживают Microsoft Intune — службу программного обеспечения, которая обеспечивает управление мобильными устройствами и приложениями. После включения и настройки Intune данные на мобильном устройстве шифруются, а само приложение Power BI не может быть установлено на SD-карту. Вы можете ознакомиться с дополнительными сведениями о [Microsoft Intune](http://www.microsoft.com/cloud-platform/microsoft-intune).
...
* **Учетные данные Power BI и домена**. Пользователи входят в Power BI с помощью адреса электронной почты. Когда пользователь пытается подключиться к ресурсу данных, служба Power BI передает адрес электронной почты для входа Power BI в качестве учетных данных. Для ресурсов, подключенных к домену (в локальной или облачной среде), адрес электронной почты для входа сопоставляется с _именем субъекта-пользователя_ ([UPN](https://msdn.microsoft.com/library/windows/desktop/aa380525(v=vs.85).aspx)) службой каталогов, чтобы определить наличие достаточных учетных данных для разрешения доступа. Для организаций, использующих рабочий адрес электронной почты для входа в Power BI (тот же адрес, который используется для входа в рабочие ресурсы, например _david@contoso.com_), это сопоставление может осуществляться беспрепятственно. Для организаций, не использующих рабочие адреса электронной почты (например, _david@contoso.onmicrosoft.com_), должно быть обеспечено сопоставление каталогов, чтобы разрешить доступ к локальным ресурсам с помощью учетных данных для входа Power BI.

* **SQL Server Analysis Services и Power BI**. Для организаций, использующих локальные службы SQL Server Analysis Services, Power BI предлагает локальный шлюз данных Power BI (который представляет собой **Gateway**, упоминаемый в предыдущих разделах).  Этот локальный шлюз данных Power BI может применить для источников данных безопасность на уровне ролей (RLS). Дополнительные сведения об RLS см. в разделе **Проверка подлинности пользователей для источников данных** выше. Можно также изучить подробную статью о [Power BI Gateway](https://powerbi.microsoft.com/documentation/powerbi-gateway-enterprise/).
...
* **Подключения, не присоединенные к домену**. Для подключений к данным, которые не присоединены к домену и не поддерживают безопасность на уровне ролей (RLS), пользователь должен предоставить учетные данные во время процедуры соединения, которые затем передаются в источник данных для установки соединения Power BI. Если разрешения достаточны, данные загружаются из источника данных в службу Power BI.
...
**Как насчет безопасности на основе ролей, совместного использования отчетов и панелей мониторинга, а также подключений к данным? Как это работает с точки зрения доступа к данным, просмотра панелей мониторинга, доступа к отчетам или их обновления?**
...
**Наши пользователи постоянно подключаются к одним и тем же источникам данных, некоторым из которых требуются учетные данные, отличные от учетных данных домена. Как можно избежать ввода этих учетных данных при каждом подключении к данным?**
...
**Какие порты используются локальным шлюзом данных и персональным шлюзом? Существуют ли имена домена, которые нужно разрешить для обеспечения взаимодействия?**

* Подробный ответ на этот вопрос приведен по следующей ссылке: [https://powerbi.microsoft.com/documentation/powerbi-gateway-enterprise](https://powerbi.microsoft.com/documentation/powerbi-gateway-enterprise)

**Как используются ключи восстановления и где они хранятся при работе с локальным шлюзом данных? Как обстоят дела с безопасным управлением учетными данными?**

* При установке и настройке шлюза администратор вводит **ключ восстановления** шлюза. Этот **ключ восстановления** используется для создания двух наборов гораздо более надежных ключей:
  ...
  Когда пользователь вводит учетные данные источника данных в пользовательском интерфейсе службы Power BI, они шифруются с помощью открытого ключа в браузере. Шлюз повторно шифрует (уже зашифрованные) учетные данные с помощью симметричного ключа AES перед сохранением данных в Power BI. Таким образом, у службы Power BI никогда нет доступа к незашифрованным данным.
...
* Шлюз поддерживает два следующих протокола связи:
  ...
  - **AMQP 1.0 – TCP + TLS**: этому порту требуются открытые порты 443, 5671–5672 и 9350–9354 для обеспечения исходящей связи. Этот протокол является предпочтительным, так как он имеет меньшие потери пропускной способности.

  - **HTTPS – WebSockets через HTTPS + TLS**: этот протокол использует только порт 443. WebSocket инициируется одним сообщением HTTP CONNECT. После установления канала связь осуществляется, главным образом, посредством TCP+TLS. Вы можете принудительно настроить шлюз для использования этого протокола, изменив параметр, описанный в статье о [локальном шлюзе](https://powerbi.microsoft.com/documentation/powerbi-gateway-onprem/).
...
* Да. Карты Bing и визуальные элементы ESRI передают данные за пределы службы Power BI для визуальных элементов, использующих эти службы. Дополнительные сведения и подробное описание исходящего трафика клиента Power BI см. в разделе [**Power BI и ExpressRoute**](https://powerbi.microsoft.com/documentation/powerbi-admin-power-bi-expressroute/).

**Как насчет национальной принадлежности данных? Можем ли мы подготовить клиенты в центрах обработки данных, расположенных в определенных географических регионах, чтобы гарантировать, что данные не покинут границы соответствующей страны?**

* Некоторые клиенты в определенных географических регионах могут создать клиент в национальном облаке, где хранение и обработка данных отделены от всех других центров обработки данных. Национальные облака имеют немного другой тип безопасности, так как отдельные доверенные лица по данным обеспечивают работу службы Power BI национальных облаков от имени Майкрософт.

  Кроме того, клиенты могут настроить клиент в определенном регионе, однако у таких клиентов нет отдельных доверенных лиц по данным, действующих от имени корпорации Майкрософт. Цены на национальные облака отличаются от общедоступной коммерческой службы Power BI. Дополнительные сведения о доступности служб Power BI для национальных облаков см. в разделе [Национальные облака Power BI](https://powerbi.microsoft.com/clouds/).

**Как корпорация Майкрософт обслуживает подключения для клиентов с подпиской Power BI Premium? Эти подключения чем-то отличаются от установленных для службы Power BI уровня, отличного от Premium?**
...
- [Национальные облака Power BI](https://powerbi.microsoft.com/clouds/)
- [Power BI Premium](https://aka.ms/pbipremiumwhitepaper)
- [Использование Kerberos для единого входа из Power BI в локальные источники данных](https://docs.microsoft.com/power-bi/service-gateway-kerberos-for-sso-pbi-to-on-premises-data)